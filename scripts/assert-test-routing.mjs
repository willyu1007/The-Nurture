...
if (
  routes.unit.length !== 74 ||
  routes.productionDb.length !== 38 ||
  routes.devHost.length !== 11 ||
  routes.scenarioService.length !== 14 ||
  routes.x5Joint.length !== 2
) {
  throw new Error(
    `Test file census changed: unit=${routes.unit.length}/74 productionDb=${routes.productionDb.length}/38 devHost=${routes.devHost.length}/11 scenarioService=${routes.scenarioService.length}/14 x5Joint=${routes.x5Joint.length}/2`,
  );
}
