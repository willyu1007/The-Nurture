...
if (
  ...
  routes.productionDb.length !== 19 ||
  routes.devHost.length !== 11 ||
  routes.scenarioService.length !== 10 ||
  routes.x5Joint.length !== 1
) {
  throw new Error(
    `Test file census changed: unit=${routes.unit.length}/50 productionDb=${routes.productionDb.length}/19 devHost=${routes.devHost.length}/11 scenarioService=${routes.scenarioService.length}/10 x5Joint=${routes.x5Joint.length}/1`,
  );
}
