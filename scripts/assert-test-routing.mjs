...
if (
  ...
  throw new Error(
    `Test file census changed: unit=${routes.unit.length}/33 productionDb=${routes.productionDb.length}/13 devHost=${routes.devHost.length}/11 scenarioService=${routes.scenarioService.length}/10 x5Joint=${routes.x5Joint.length}/1`,
  );
}
