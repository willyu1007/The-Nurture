...
if (
  ...
  throw new Error(
    `Test file census changed: unit=${routes.unit.length}/83 productionDb=${routes.productionDb.length}/42 devHost=${routes.devHost.length}/11 scenarioService=${routes.scenarioService.length}/14 x5Joint=${routes.x5Joint.length}/2`,
  );
}
