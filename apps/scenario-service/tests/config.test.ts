...
describe("scenario-service configuration", () => {
  ...
  it("loads the teacher organization-owner gate only from an exact true literal", () => {
    expect(
      loadScenarioServiceConfig({
        NURTURE_TEACHER_ORGANIZATION_OWNER_ENABLED: "true",
  ...
  it("loads the teacher media-association gate only from an exact true literal", () => {
    expect(
      loadScenarioServiceConfig({
        NURTURE_TEACHER_MEDIA_ASSOCIATION_OWNER_ENABLED: "true",
