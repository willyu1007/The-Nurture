...
- State: `G4_0A_INVENTORY_PASS` on 2026-08-01. 0B～0E have since frozen and
  exited; 0F scope and all three unit contracts are accepted, and its 0G audit
  passes after repair. The 0F Exit remains open. Live branch state is
...
| Institution capability set | T-007 | 0B～0E are frozen and exited; 0C/0D are implemented at I1 and G4-D has I1 plus default-off I2. 0F scope and all three units are accepted in [`64`](./64-g4-0f-scope-freeze.md)–[`67`](./67-g4-0f-3-citation-answer-safety-freeze.md); 0G audit passes in [`68`](./68-g4-0g-0f-audit-record.md), while Exit remains | `DEFINED_UNQUALIFIED`; unit/audit gap closed, branch release still open | G4-A～F | any accepted freeze-record drift reopens the affected branch only |
