...
- Guardian mappings are `confirm_family_enrollment -> nurture.family_care.confirm_enrollment`, `suspend_family_enrollment -> nurture.family_care.suspend_enrollment`, `resume_family_enrollment -> nurture.family_care.resume_enrollment`, `confirm_enrollment_transfer -> nurture.family_care.confirm_enrollment_transfer`, `decline_enrollment_transfer -> nurture.family_care.decline_enrollment_transfer`, `withdraw_family_enrollment -> nurture.family_care.withdraw_enrollment`, `confirm_child_link_grant -> nurture.family_care.confirm_grant`, and `replace_child_link_grant -> nurture.family_care.replace_grant`; existing `revoke_child_link_grant -> nurture.family_care.revoke_grant` remains unchanged. Family pause/transfer/withdrawal actions remain design-only until their C-2f implementation.
...
- Preconditions MUST cover active source Enrollment, zero holds at proposal/confirmation, hold/version drift, same Institution/different Groups, source active/paused/archived Group transfer-out, target active readiness/Lead/policy/gates/capacity, wrong/deleted scope, pending conflict, duplicate current Enrollment, and cross-Institution denial.
- Roster/schema tests MUST cover additive exact `Enrollment.rosterEntryId`, target creation only on confirmation, no target row on cancel/decline/expiry/fault, source `transferred` history, current family-safe target label, no institution prefill copy, unique binding, and activation/manual-reconciliation behavior for missing or ambiguous legacy evidence.
...
- Scope-classification tests MUST prove one same-workspace/current-Family-resolved ChildCareProcess can remain longitudinal while stage episodes and Institution-local Enrollments have separate identities, owners, lifecycles, and authorization predicates. Tests MUST reject a global Child/Process identity or an Institution-owned longitudinal profile.
...
- Cross-workspace negative tests MUST deny raw-id linking, fuzzy/global match, automatic reuse/export/import/migration, and possible-match disclosure for same adult account, child name, birth fact, contact, media, family relation, or roster. Target-local identity creation under a future protocol MUST remain unavailable in Pilot.
...
Pilot-0-C2f-4-3 future cross-workspace protocol-boundary test refinement:

- Pilot-absence tests MUST prove there is no cross-workspace child lookup, candidate query, fuzzy/global matcher, raw-link field, export/import action, portability route/token, manifest capability, dataClass, handler, repository, ledger, Handoff declaration, notification, or executable runtime path. Same adult, name, birthDate, contact, media, roster, and raw id inputs MUST reveal no candidate or possible-match signal.
- Actor tests for a future v1 MUST allow issue/revoke only from any current exact-source-Family Guardian and require trusted My-Chat proof that the same canonical adult reauthenticated in the exact target workspace. Tests MUST deny hidden primary/unanimity/Grant-owner/invitation-owner hierarchy, stale/exited Guardian, another adult, delegated recipient, Institution Admin, Caregiver/Lead, Technical Operator, service/Host admin, AI, raw-id caller, client same-person claim, and copied Host binding.
- Host-boundary tests MUST prove My-Chat owns raw account/session and workspace authentication and emits only trusted opaque same-adult/target bindings. Nurture persists no raw My-Chat account/contact identity and independently resolves source Participant/Guardian plus target eligibility. Name/contact/birth/media similarity cannot replace either proof.
- Target-creation tests MUST use a five-minute strong-confirmation context and atomically create one fresh target-local Child, ChildCareProcess, Family, first Guardian RoleAssignment, and CommandExecution with new ids and no source authority. Fault injection at every target boundary leaves no authority-free or partial aggregate. Source relationship metadata is not copied; target relationship input is independently confirmed.
- Existing-target tests MUST prove v1 never searches, ranks, suggests, selects, links, overwrites, deduplicates, or merges a target profile. Confirmation states that the operation always creates a new profile and directs a user who already created that child's target profile to stop. No Technical Operator or hidden reconciliation bypass exists.
- Allowlist tests MUST permit only required Guardian-confirmed `displayName` and optional `birthDate`. BirthDate starts unselected and requires explicit source selection plus exact target review. Snapshot tests bind protocol version, exact selected field names/values, source Child aggregate version, and canonical hash. Source value change, aggregate-version drift, redaction, deletion, or selection/hash drift before consume makes the snapshot stale and creates no target fact.
- Forbidden-payload tests MUST reject `profileBasicsPayload`, `careContextSummary`, `careContextPayload`, StageEpisode/currentStageKey/history, relationship metadata, Guardian/Participant/account/contact, Institution/CareGroup/Roster/Enrollment/lineage, Grant/Thread/Message/Receipt/Item/Attention, care/media/pregnancy/health/material, notification/Handoff, audit bodies, raw ids, and technical state at codec, policy, repository, log, and presenter boundaries. Target stage remains unset until a separate target-local C-2f-4-1 command.
- Intent-lifecycle tests for future implementation MUST fix `expiresAt=issuedAt + 7×24 hours`, persist only `pending|consumed|revoked|superseded`, derive expiry from database time, and prohibit in-place expiry extension. Target, recipient, field, value, source Child aggregate version, or expiry change supersedes/reissues with a new identity and leaves at most one effective pending Intent for the exact source/target purpose.
- Token tests MUST store only a token hash and bind purpose, protocol version, source scope, target workspace, exact opaque same-adult recipient, expiry, and single-use identity. Raw token MUST be absent from database/log/metrics/trace, non-comparable across workspaces, invalid for forwarding or another target, and mapped to generic unavailable on wrong recipient/workspace/purpose/version/lifecycle. Target rows expose no source raw id or stable global child key.
- Owner-reread tests MUST cover issue, open, review, confirm, replay, and revoke. Every step rereads source Host/Nurture identity, current Guardian/Family/Child/Process, selected values and Child aggregate version/lifecycle, Intent/policy/expiry, same-adult Host attestation, target workspace eligibility, and confirmation context. Source role loss, last-authority loss, disable/delete/redaction, selected-value or aggregate-version drift, target mismatch, expiry, or policy denial fails before target creation without disclosing the exact cause.
- Revoke/consume race tests MUST allow any current exact-source-Family Guardian to revoke a pending Intent and provide one Nurture-owned linearization point. Revoke/expiry/redaction/policy denial winning before consumption yields zero target facts; consumption winning yields one target aggregate and an immutable consumed result. A later revoke cannot report target deletion or rewrite the winner.
- Replay/fault tests MUST inject response loss and faults before/after the consumption decision and each target write. Exact retry returns one original target result; changed payload/target/recipient conflicts; no second aggregate or ambiguous bearer reuse occurs. Reconciliation uses only the bounded Nurture portability ledger and never replays a Workflow Step/Handoff/Outbox as business authorization.
- Post-import tests MUST prove source revoke, redaction, Guardian loss, workspace closure, or deletion cannot mutate or distributed-delete committed target facts. Target correction/deletion follows only current target Guardian policy. Both confirmation surfaces disclose this no-recall boundary and tests reject copy that implies continued source control or guaranteed remote erasure.
- Ownership tests MUST keep minimum snapshot, lifecycle, consumption/idempotency, target creation, and business audit in Nurture; raw account/auth/session, workspace access, opaque recipient binding, and refs-only delivery stay in My-Chat. Tests MUST reject portability bodies or child-identity claims in My-Chat Workflow Step, Handoff, Outbox, notification, provider payload, logs, or Admin controls.
- Planning-boundary tests MUST prove C-2f-4-3 changes no schema/source/runtime/manifest/database/traffic and creates no current action/route/token/ledger/capability. C-2f-5 may lock current-Pilot result/recovery/surface/Handoff behavior but cannot activate, reuse, or implicitly implement portability.
...
- Recipient snapshot tests MUST use one stable request/draft key per exact RoleAssignment at business commit. Later-added Guardians MUST NOT receive old events; removed/suspended/exited recipients MUST stop on current reread. Zero eligible recipients MUST persist `[]`. Exact replay MUST return byte-equivalent snapshots, and changed recipient/source/expiry MUST conflict rather than mutate history.
...
- Open tests MUST enforce My-Chat exact recipient/workspace/Notification validation, eligible Handoff reread, current Nurture owner resolver, destination-bound `open_notification`, and destination reread in that order. Provider delivery and Host read/unread MUST NOT mark a Nurture transfer/Enrollment/read/ack outcome. Stale/revoked/terminal/policy-changed opens MUST render only current safe state.
...
- Shared-contract tests MUST use generic `subject` and MUST reject Base/My-Chat imports or fields tied to `NurtureChild`, `NurtureChildCareProcess`, Family, CareGroup, Enrollment, or other scenario-owned identity. Product copy may say child without changing the contract type or creating a Host Child SSOT.
...
- Guardian-path tests MUST require the current exact Participant, Guardian RoleAssignment, Family, and ChildCareProcess. Caregiver-path tests MUST require current exact Caregiver/Lead RoleAssignment, CareGroup, and eligible Enrollment before returning a collection. Institution-path tests MUST require current Institution Admin RoleAssignment and exact Institution/CareGroup/Enrollment scope. Technical Operator and ordinary workspace admin MUST receive no business subject context.
...
- Multi-child tests MUST cover zero, one, and several current subjects for one Guardian; one Family with two Guardians; one Caregiver collection across all three Pilot child scopes; and Institution collections without protected family content. Current relationships, not account history or UI selection, determine membership.
...
- Provider tests for future implementation MUST require versioned scenario ownership, list current reachable contexts, and resolve/recheck exact opaque context. Responses MAY carry only workspace/scenario-bound opaque context, scope kind, display-safe owner copy, route class, and freshness/version evidence. Raw ids, profile bodies, protected relationship metadata, stable cross-scenario keys, action authority, and client-authored membership MUST fail codec/privacy validation.
- Host-boundary tests MUST prove Pilot uses live owner resolution and creates no canonical My-Chat Subject/account-subject authorization table. Any future index remains out of scope and cannot be inferred from Notification, Chat history, analytics, workflow refs, contact books, or cached recent subjects.
...
- Isolation tests MUST deny cross-workspace/scenario comparison, same-child inference, global search, merge, raw linking, portability, child-account impersonation, and context forwarding. Subject handles MUST be absent from URL, durable Chat copy, Notification/provider payload, logs, analytics, metrics dimensions, and Technical Admin relationship queries.
...
- Pilot-authenticator tests MAY cover one separate high-entropy static caller credential mapped to an exact environment/audience/subject, current/next rotation, and network source. No environment may enable both static and workload-token authenticators or fallback between them; `NURTURE_INTERNAL_SERVICE_TOKEN` remains owner-read-only.
...
- Product-surface tests MUST cover `nurture_chat`, family board/workbench, teacher board, and institution board/workbench. General Chat cannot produce `nurture_chat` without explicit business transition; Institution-only role through Chat, wrong role/surface, client surface injection, and product-key switching MUST remain unavailable after current owner resolution without granting a role.
...
- Surface-convergence tests MUST exercise Chat, family board/workbench, teacher board, and institution board/workbench against one owner presenter. Eligible subset/layout changes are allowed only when owner-declared; facts, safe reason, target state, token, action key, and lifecycle MUST remain one canonical owner result rather than per-surface state machines.
...
- My-Chat conformance MUST include six product surfaces by six semantic block kinds, complete compatible renderer coverage, Institution-board action rejection, owner order/copy/tone preservation, freshness and invalidation clearing, accessibility/device/locale/focus coverage, all Host persistence destinations, content-free Chat rehydration, capability-off, legacy unchanged, and no partial/alternate fallback.
...
- Planning-exit tests MUST report C-3-0c as design/evidence-specification complete but implementation/adoption/enablement incomplete. No contract package, manifest, source, schema, migration, route, UI, renderer, runtime, cache, secret, environment, capability, provider, database, or traffic changed. C-3-0d is the next decision checkpoint, and C-3-0e retains actual complete-IIB adoption evidence.
