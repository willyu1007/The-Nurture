...
- Identity-chain tests MUST bind Nurture Participant by exact workspace plus canonical account ref. Host actor ref is audit provenance only. Zero Participant returns generic unavailable; duplicate/mismatched Participant is a fail-closed data defect; neither case may fall back to actor id, email/contact, account history, another workspace, or client selection.
