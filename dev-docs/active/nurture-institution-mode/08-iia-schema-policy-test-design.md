...
- Planning-boundary tests MUST prove C-3-0c-0 changes task/governance documentation only. No contract package, manifest, source, schema, migration, route, UI, renderer, runtime, cache, secret, environment, capability, provider, database, or traffic change exists. C-3-0c-1 subject-provider wire is next.
