...
Teacher mobile is a surface composition, not a canonical business capability. The
surface may render these capabilities together as one teacher work surface, but the
surface must not own their business facts, policies, or lifecycles.
...
- `child_media_attribution` is first-slice institution value, but
  `child_media_attribution` is not required to prove the minimum `family_to_org`
  communication loop.
...
- Guardian and caregiver Nurture work MAY enter through My-Chat Chat. My-Chat MAY process the current turn to decide whether the turn belongs to Nurture, but the route decision only selects `scenario=nurture`; the route decision does not resolve the Nurture actor role, work scope, child target, business intent, grant, or propagation direction.
...
| effect key/ref | A child row, receipt, task, artifact, or other command effect | Owning domain uses the effect key/ref for effect-level uniqueness; the key/ref is not a second command unless the effect has an independent commit/retry boundary |

Lifecycle and derivation:

- The current entry owner MUST supply a stable `invocationRequestId`. My-Chat maps message/form/action client identity into `invocationRequestId` and preserves the identifier on infrastructure retry; identifier assignment does not decide whether a Nurture command exists.
