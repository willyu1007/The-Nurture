...
- **Current checkpoint:** Pilot-0-C in progress; C-3-0b authenticated ingress, C-3-0c subject-aware presentation, and C-3-0d action execution/recovery planning are complete. C-3-0d locks atomic capability, static direct-empty/claimed-Step drivers, pre-claim Step binding, owner-derived identity, typed Participant actor evidence, atomic transaction/replay/reconciliation, exact action matrix, named adoption evidence, and no fallback; C-3-0e protected/offline/complete adoption is next
...
| C-3 Guardian/Caregiver operational IIB | **IN PROGRESS — C-3-0d COMPLETE** | C-3-0b/c/d lock trusted ingress, subject presentation, domain-action driver/identity/transaction/recovery, exact adoption identities, and negative conformance without claiming implementation. C-3-0e protected/offline/complete adoption is next. |
...
| Checkpoint | State | Decision boundary |
| --- | --- | --- |
| C-3-0 shared subject-aware IIB baseline | **IN PROGRESS — C-3-0d COMPLETE** | Account–Subject reachability, authenticated ingress, presentation, and action execution/recovery gates are locked; protected-data/offline and actual complete-IIB adoption remain C-3-0e. |
...
| C-3-4 cross-surface/result/notification continuity | OPEN | Draft stay/discard, route/view continuation, response-loss result recovery, Notification/deep-link owner reread, and privacy/offline behavior. |
| C-3-5 Guardian/Caregiver evidence and exit | OPEN | Action/surface conformance plus B3 J1-J4 rendered, fault, privacy, and three-child-scope evidence without claiming Institution C-4 closure. |
...
2. Product experience remains child-centered, while reusable Base/My-Chat contracts use generic `subject`. A Nurture adapter may map `subject` to `NurtureChildCareProcess`; Base/My-Chat cannot import, persist, rename, or reinterpret that domain type. The authenticated adult account is an actor, not the child subject, and C-3-0a creates no child login.
...
4. Nurture owns the complete relationship graph. Guardian path is Participant -> current exact Guardian RoleAssignment -> exact Family -> ChildCareProcess. Caregiver path is Participant -> current Caregiver/Lead RoleAssignment -> exact CareGroup -> current eligible Enrollment -> ChildCareProcess collection. Institution Admin path is Participant -> current Institution Admin RoleAssignment -> exact Institution -> scoped CareGroups/current eligible Enrollments -> collection. Technical Operator has no business subject relationship.
...
8. My-Chat may aggregate only workspace/scenario-bound opaque entries, scope kind, display-safe owner copy, generic route class, and freshness/version evidence for discovery/navigation. Pilot uses live owner resolution and creates no canonical Host Subject table, account-subject authority ledger, or stable cross-scenario correlation. A future refs-only index would be a revocable non-authoritative projection requiring separate privacy and lifecycle review.
...
10. Subject context is not global child identity, custody proof, raw business reference, workspace bridge, cross-scenario link, portability token, role delegation, authority transfer, or account impersonation. Cross-workspace/cross-scenario same-child inference, search, matching, merging, and automatic migration remain forbidden until a separately versioned consent protocol exists.

The authoritative reachability examples are:

| Actor context | Required owner path | Returned scope | Explicit denial |
| --- | --- | --- | --- |
| Current Guardian | Account -> Participant -> Guardian RoleAssignment -> Family -> Process | One or more `single_subject` options or a Family-scoped collection for navigation. | Another Family, stale/exited Guardian, guessed Child, account-history inference. |
| Current Caregiver/Lead | Account -> Participant -> exact CareGroup role -> current eligible Enrollments | CareGroup `subject_collection`. | Outside-group child, roster-only protected access, direct family relationship, bulk authority. |
...
15. A temporary isolated-Pilot static caller authenticator may exist only with a separate high-entropy secret, exact environment/audience/subject mapping, network source restriction, and current/next rotation. One environment selects exactly one caller authenticator and never falls back. Current `NURTURE_INTERNAL_SERVICE_TOKEN` remains activation owner-read-only and cannot authenticate ordinary scenario invocation.
...
| --- | --- | --- |
| `product_surface` | `nurture_chat`, `family_board`, `family_workbench`, `teacher_board`, `institution_board`, `institution_workbench` | Current Nurture product presentation/action context. |
...
1. `chat_text` carries bounded text and optional bounded `client_message_id`.
...
C-3-0b-4 adoption evidence is ingress-only. C-3-0e retains complete subject-provider/presentation/domain-action/protected-body/draft/cache/offline adoption and final C-3-0 evidence. Closing C-3-0b therefore proves a coherent authenticated-ingress design, not an implemented or user-operable Guardian/Caregiver IIB.
...
| Host adult invitation, C-0 bootstrap, Technical Operator, portability | Outside ordinary domain-action driver registry | Retain separately owned identity, provisioning, operations, or future protocol. |
...
C-3-0d is complete as a design and implementation-evidence specification only. C-3-0d completion does not authorize implementation, database change, capability enablement, Pilot-1, or traffic. C-3-0e is next and retains protected body/media transport, draft lifecycle, cache/offline policy, actual Base/My-Chat/Nurture adoption, complete-IIB evidence, and final shared-baseline exit.
...
| Pilot-0-C — IIB and onboarding closure contract | **In progress — C-3-0d complete** | C-3-0b/c/d lock ingress, presentation, and action execution/recovery through atomic capabilities, exact identities/drivers/transactions, typed actor evidence, ordered three-repo adoption, legacy/no-fallback behavior, and conformance. C-3-0e is next; C-4 Institution IIB/closure evidence remains open. |
