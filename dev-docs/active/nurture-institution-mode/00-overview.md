...
- **Updated:** 2026-07-19
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C3-0c-4 conformance/adoption evidence after C-3-0c-3 locked renderer and presentation-persistence behavior. Six Nurture product surfaces map to My-Chat Chat semantic panel, role-board, or domain-workbench renderers without placing UI primitives on the semantic wire. The Institution board is read-only; the Teacher board keeps complete owner-paginated history. Pilot arbitrary search/sort/compound filters are deferred to a later owner query-control contract. Presentation data defaults to ephemeral memory; My-Chat may persist only a strict content-free Host shell and Chat rehydration marker, while protected/owner/action material remains forbidden from Host persistence. A maximum 60-second foreground freshness lease, immediate known invalidation, no stale/cache/legacy/offline fallback, and renderer accessibility are locked. Draft/confirmation execution remains C-3-0d; protected bodies/media/offline remain C-3-0e. This checkpoint is planning-only: no contract, manifest, source, schema, migration, route, UI, renderer, runtime, cache, secret, capability, provider, database, or traffic changed. C-3-0c-4 through C-3-0e, Guardian/Caregiver IIB, C-4 Institution IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4 and all external enablement remain separately unauthorized.
