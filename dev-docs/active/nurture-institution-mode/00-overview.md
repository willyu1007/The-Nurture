...
- **Updated:** 2026-07-18
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C2f-3c fresh re-entry/history convergence after C-2f-3b locked one-transaction terminal closure. All Enrollment topology commands now use one global `context/actor -> Enrollment -> Institution/CareGroup -> Hold -> TransferIntent -> RosterEntry -> ordered Grant -> Thread -> dependents` lock order; this explicitly corrects C-2f-2's earlier Intent-first wording. Additive terminal audit binds Enrollment reason/actor/RoleAssignment/Execution/time; Hold `closed` records database time and owning Execution without release actor; topology-invalidated Grant uses `revoked` with exact Enrollment cause, null `revokedByParticipantId`, and CascadeAudit/Execution evidence. Pending TransferIntent becomes `invalidated(source_enrollment_terminal)`, typed InteractionContext dependencies include Enrollment, Thread closes, and roster becomes matching noncurrent history. One Serializable transaction preflights the aggregate C-2e hard cap, obtains database `T`, consumes context, terminalizes Enrollment, closes holds/intents/all status-active Grants and dependents, closes Thread/roster, writes audits/Execution, and asserts zero actionable survivors. Elapsed status-active Grants converge as expiry rather than false topology revoke. Exact replay is immutable; fresh same-cause duplicate is `already_satisfied` without audit rewrite; different terminal causes and all lifecycle/work races are first-commit-wins. Any fault/overflow/integrity defect rolls back all and triggers Pilot stop/manual reconciliation. No remote call or Host delivery is a commit condition. C-2f-3c now owns fresh roster/invitation/Enrollment identities, explicit re-entry provenance, and retained history; C-2f-4/C-2f-5, operational IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4, database apply, ACR publication, secret configuration, capability activation, external traffic, staging, production, and GA remain closed until separate approval.
