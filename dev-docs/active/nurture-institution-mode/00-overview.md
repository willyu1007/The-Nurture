...
- **Updated:** 2026-07-19
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C3-0b-4 client echo, safe denial, audit, adoption, and negative closure evidence after C-3-0b-3 locked the complete ingress-variant model. The signed ordinary boundary now uses a versioned discriminated ingress context: registered product surfaces (`nurture_chat|family_board|family_workbench|teacher_board|institution_board|institution_workbench`), Host transitions (`notification_open|invitation_continuation`), and runtime ingress (`workflow_worker`). Product and Notification calls use `my-chat-api + interactive_session`; durable calls use `my-chat-workflow-worker + durable_run_actor`; caller/origin/ingress/operation combinations are closed and cannot fall back. Notification performs a recipient-bound read-only owner transition before a fresh destination-surface invocation. Worker replay uses the current durable Run actor plus a separately claimed original Step and never impersonates a UI surface. Post-accept onboarding uses the same signed human principal but is the sole exact-invitation-bound exception that may create/reuse a missing Participant in its atomic onboarding transaction; it exposes no ordinary Subject data before commit. C-0 provisioning and Technical Operator owner recovery use separate versioned endpoints, audiences, callers, gates, and non-business authority contexts rather than `ScenarioHumanPrincipalV1`. This checkpoint is planning-only: no contract, manifest, source, schema, migration, route, runtime, secret, capability, provider, database, or traffic changed. C-3-0b-4, later Guardian/Caregiver IIB, C-4 Institution IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4, database apply, ACR publication, secret configuration, capability activation, external traffic, staging, production, and GA remain closed until separate approval.
