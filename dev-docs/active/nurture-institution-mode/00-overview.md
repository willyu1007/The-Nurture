...
- **Updated:** 2026-07-18
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C2f-4-2 same-workspace multi-Institution visibility/concurrency after C-2f-4-1 locked the family-owned stage lifecycle. Additive `NurtureChildCareStageEpisode` is the linear stage SSOT with a versioned catalog, one current row, exact predecessor, database-time transitions, Guardian/RoleAssignment/Execution audit, and explicit change/correction/clear semantics. Any current exact-Family Guardian may independently use `update_child_care_stage -> nurture.family_care.update_child_care_stage` from Chat, family board, or family workbench through a five-minute strong confirmation; no primary/unanimous, Institution, Caregiver, Operator, AI, age, birthday, roster, or inferred authority exists. Stable coarse keys follow the product stage contract; optional fine keys are allowlisted. `currentStageKey` is only the coarse current-Episode projection and mismatches fail closed. Legacy non-null projection without a proven Episode is quarantined for Guardian reconfirmation, not backfilled as truth. Stage update changes no Enrollment or prior artifact and emits no implicit Institution visibility. C-2f-4-2 must now lock what each Institution and the family timeline may see when multiple Institution Enrollments coexist. C-2f-4-3 retains the future cross-workspace protocol boundary; C-2f-5 retains exact result/notification/Handoff. Operational IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4, database apply, ACR publication, secret configuration, capability activation, external traffic, staging, production, and GA remain closed until separate approval.
