...
- **Updated:** 2026-07-19
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C3-0b-3 ingress path variants after C-3-0b-2 locked the private invocation boundary. Every subject-aware call now requires an independently authenticated My-Chat workload plus an ES256-signed, request-bound `ScenarioPrivateInvocationV1`; the signed `ScenarioHumanPrincipalV1` contains only exact My-Chat User, human Actor, Workspace, and `interactive_session|durable_run_actor` origin. A 60-second envelope, caller/origin/audience/contract binding, atomic single-use nonce, exact codec, and separate business `command_request_id` prevent transport replay from becoming business identity or blocking deterministic CommandExecution recovery. `my-chat-api` and `my-chat-workflow-worker` are the only initial workload subjects; current `NURTURE_INTERNAL_SERVICE_TOKEN` stays owner-read-only. Private validation terminates before Participant lookup, Nurture binds Participant from Workspace plus User, Host Actor remains provenance, and activated vNext cannot fall back to `actor_id`, default workspace, broad `client_surface`, or legacy internal handlers. This checkpoint is planning-only: `trusted_scenario_invocation_v1`, keys, nonce store, schema, routes, runtime, secrets, capabilities, and traffic were not implemented or enabled. C-3-0b-3 must now classify interactive, Notification, durable replay, invitation continuation, provisioning, and Technical Operator paths; C-3-0b-4 still owns client echo allowlists, public denial mapping, audit retention, and cross-repo adoption evidence. C-3 Guardian/Caregiver IIB, C-4 Institution IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4, database apply, ACR publication, secret configuration, capability activation, external traffic, staging, production, and GA remain closed until separate approval.
