...
- **Updated:** 2026-07-17
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C2e-1 Grant review/confirming-Guardian convergence after C-2e-0 locked one authorization path: current Host/Nurture identity, Participant/RoleAssignment, exact Enrollment/CareGroup, exact current or original Grant, Thread/business lifecycle, policy, and redaction are re-read authority; ThreadParticipant is optional routing/UI/read-state projection that can neither grant nor deny business permission. A missing projection row does not block an otherwise authorized Guardian/Caregiver, while a stale or forged row never preserves or creates access. C-2e implementation must remove `thread_membership_active` as a hard gate while retaining exact Thread existence/scope/lifecycle checks. Transfer/exit stays C-2f. Guardian/Caregiver operational IIB, topology/operations, and final Go/No-Go remain open. The authorization remains readiness-only: Pilot-1 through Pilot-4, database apply, ACR publication, secret configuration, capability activation, external traffic, staging, production, and GA remain closed until separate approval.
