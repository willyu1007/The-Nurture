...
- **Updated:** 2026-07-19
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C3-0c-1 subject-provider wire contract after C-3-0c-0 locked the presentation pipeline and three-repo ownership boundary. A verified My-Chat invocation reaches the Nurture owner resolver/provider; Nurture returns only a current display-safe semantic presentation; My-Chat maps that presentation through generic Chat/card/list/detail/workbench renderers without interpreting Nurture role, Subject, lifecycle, safe reason, or action availability. Chat remains an AI conversation plus generic structured UI, and AI narration may use only the current display-safe owner result. Presentation never authorizes: render, prepare, submit, retry, and navigation reread current Host and Nurture facts. Host persistence of raw Subject ids, relationship authority, protected bodies, owner tokens, or inferred business state is forbidden; exact opaque subject-provider DTOs remain C-3-0c-1, block/action vocabulary remains C-3-0c-2, renderer/persistence classes remain C-3-0c-3, and conformance/adoption remains C-3-0c-4. This checkpoint is planning-only: no contract, manifest, source, schema, migration, route, UI, runtime, cache, secret, capability, provider, database, or traffic changed. C-3-0c-1 through C-3-0e, Guardian/Caregiver IIB, C-4 Institution IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4 and all external enablement remain separately unauthorized.
