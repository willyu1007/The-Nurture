...
- **Updated:** 2026-07-18
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C2f-2 same-Institution CareGroup transfer convergence after C-2f-1 locked reversible Enrollment pause/resume. Additive `NurtureEnrollmentPauseHold` rows preserve one active family-side and one active institution-side restriction, with `active|released|closed`, exact actor/RoleAssignment/Execution/database-time audit, safe fixed reasons, no auto-expiry, and no free text. Holds are restriction authority while Enrollment `status` is an atomic aggregate; mismatches fail closed. Any current exact-Family Guardian may operate the shared family side, and any current exact-scope Institution Admin may operate the shared institution side, but neither may release the other side. Guardian surfaces use new `suspend_family_enrollment` / `resume_family_enrollment`; Institution reuses the B3-1d `suspend_enrollment` / `resume_enrollment` keys, whose resume releases only the institution hold and does not promise global active. Both actions require current five-minute confirmation. Pause preserves business facts and Grant time, blocks current cross-role use, invalidates stale contexts through Enrollment version, and creates no permanent cascade or automatic replay. Serializable first-commit-wins requires stale cross-side concurrency to refresh and reconfirm; it cannot silently merge. C-2f-2 now owns transfer proposal/confirmation, old/new Enrollment lineage, roster handling, and atomic old-work closure. C-2f-3 through C-2f-5, operational IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4, database apply, ACR publication, secret configuration, capability activation, external traffic, staging, production, and GA remain closed until separate approval.
