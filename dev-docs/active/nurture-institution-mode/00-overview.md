...
- **Updated:** 2026-07-19
- **Owner:** willyu1007
- **Next step:** Begin Pilot-0-C3-0d action-execution and recovery discussion after C-3-0c presentation planning completed. C-3-0c-4 locks additive Base provider/presentation contracts, atomic capability `scenario_subject_presentation_v1` with dependency on `trusted_scenario_invocation_v1`, exact Base -> My-Chat -> Nurture revision/source/module/renderer evidence, mechanically derived pre-activation projection, legacy compatibility, no fallback, and four-layer conformance. The existing `0bd892...` hash remains evidence only for the reviewed handoff contract and MUST NOT be relabeled as presentation adoption; a separately named `scenario_interface_source_v1` source set is required during implementation. C-3-0c completion means design and implementation gates are complete, not that contracts, providers, renderers, capability, or a workspace allowlist exist. C-3-0d still owns domain action driver/idempotency/recovery and C-3-0e owns protected bodies/media/offline plus complete-IIB adoption. This checkpoint is planning-only: no contract, manifest, source, schema, migration, route, UI, renderer, runtime, cache, secret, capability, provider, database, or traffic changed. C-3-0d/e, Guardian/Caregiver IIB, C-4 Institution IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4 and all external enablement remain separately unauthorized.
...
- The current path-content and logical-source hashes predate C-3 trusted ingress and subject presentation. They remain historical handoff evidence only. Future C-3 implementation MUST add the separately named `scenario_interface_source_v1` adoption source set and MUST record Scenario module and Host renderer conformance identities independently.
