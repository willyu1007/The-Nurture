...
- **Updated:** 2026-07-18
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C2f-4 next-stage and cross-scope portability after C-2f-3c locked fresh same-Institution re-entry. Institution workbench reuses `initiate_enrollment` from an exact historical Enrollment; the exact invited current Guardian reuses `confirm_family_enrollment` on any Guardian surface. Re-entry keeps Child/ChildCareProcess/Family/current Guardian identities but requires a fresh unlinked RosterEntry, seven-day Enrollment Invitation, five-minute context, CommandExecution, and active Enrollment. The new Enrollment alone stores one-way `predecessorEnrollmentId` plus `continuityKind=fresh_reentry`; the same pair covers transfer with `care_group_transfer` and supersedes the unimplemented `supersedesEnrollmentId` proposal, preventing dual lineage SSOT. The predecessor must be the fully closed, same-workspace/process/Institution terminal leaf with no successor or current same-Institution Enrollment. Re-entry may target the same or another ready CareGroup in that Institution, never restores old Grant/Thread/work, and creates no new Grant or Thread. Old and new Enrollment episodes stay separate; safe terminal summaries and independently authorized same-side history remain available, while old cross-role bodies stay unavailable and exact terminal actor remains audit-only. Cross-Institution/next-stage portability stays C-2f-4; exact result/notification/Handoff stays C-2f-5. Operational IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4, database apply, ACR publication, secret configuration, capability activation, external traffic, staging, production, and GA remain closed until separate approval.
