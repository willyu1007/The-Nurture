...
- **Updated:** 2026-07-18
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C3 Guardian/Caregiver operational IIB after C-2f-5 completed the C-2f lifecycle planning contract. Lifecycle commands now separate immutable business outcome (`applied|already_satisfied`), response disposition (`executed|replayed`), and current owner-presented result. Exact output refs remain server-side recovery evidence and never become client locators. Current presenters reread authoritative state; response loss recovers through the original CommandExecution and same durable Step, without compensating committed Nurture facts. The existing `user_attention` contract remains unchanged; a future additive `guardian_relationship_attention` is limited to Guardian transfer review and irreversible relationship termination, with per-RoleAssignment recipient snapshots, generic refs-only notification, and current owner reread. Pause/resume, transfer completion, re-entry confirmation, stage changes, and portability remain explicit-empty or without a current Host effect as previously locked. This checkpoint is planning-only: no manifest, source, schema, runtime, database, capability, provider, or traffic changed. Operational IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4, database apply, ACR publication, secret configuration, capability activation, external traffic, staging, production, and GA remain closed until separate approval.
