...
- **Updated:** 2026-07-18
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C2f-4-1 stage fact, Guardian authority, and StageEpisode lifecycle after C-2f-4-0 locked the portability classification. `NurtureChildCareProcess` is the same-workspace longitudinal spine, a stage episode is a family-owned phase, and an Enrollment is one Institution-local relationship episode. A different Institution always uses the normal fresh roster/invitation/Guardian-confirmed Enrollment flow against the current-family-selected Child/Process; it may coexist with another Institution Enrollment, does not auto-end the old relationship, creates no cross-Institution predecessor edge, and inherits no Grant, Thread, content, role, or authority. A next-stage change keeps Child/Process and neither creates nor terminates Enrollment; a combined stage/new-Institution journey remains two separately authorized effects with no distributed transaction. Cross-workspace reuse, fuzzy/global child matching, raw linking, and automatic migration remain Pilot `NO-GO`, including when the same My-Chat adult exists in both workspaces. The current mutable `currentStageKey` cannot become stage-history authority; C-2f-4-1 must decide the versioned stage fact/projection and exact Guardian command. C-2f-5 still owns exact result/notification/Handoff. Operational IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4, database apply, ACR publication, secret configuration, capability activation, external traffic, staging, production, and GA remain closed until separate approval.
