...
- **Updated:** 2026-07-19
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C3-0b-2 private service caller and trusted-principal envelope after C-3-0b-1 locked three public Host context modes. `platform_general` supports daily generic Q&A without a business workspace and can never call Nurture; a personal-workspace storage partition is not business context. `workspace_business` requires one resource-bound, shell-selected, or unambiguous explicitly business-scoped workspace plus current membership and scenario/cohort/surface gates. Moving from general Chat to private Nurture is always an explicit mode transition—even with one eligible workspace—and opens a workspace-scoped conversation carrying only the current confirmed intent by default. `invitation_acceptance` authenticates the exact recipient without pre-existing target membership, atomically accepts the Host invitation/membership, and then offers a separate gated business continuation; it does not auto-run Nurture or form a cross-database transaction. Session expiry, membership loss, multi-tab drift, silent personal fallback, owner workspace inference, and client surface/workspace authority fail closed. This checkpoint is planning-only: exact private caller/principal wire semantics remain C-3-0b-2, and no contract, invitation model, manifest, source, schema, route, runtime, database, secret, capability, provider, or traffic changed. C-3 Guardian/Caregiver IIB, C-4 Institution IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4, database apply, ACR publication, secret configuration, capability activation, external traffic, staging, production, and GA remain closed until separate approval.
