...
- **Updated:** 2026-07-18
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C2f-4-3 future cross-workspace protocol boundary after C-2f-4-2 locked same-workspace multi-Institution visibility/concurrency. A current Guardian may aggregate the family-owned StageEpisode timeline and safe Institution/Group/join/left/status episode summaries for the exact ChildCareProcess, while each protected body remains under its original Enrollment/Grant/role/redaction/retention policy. Institution Admin and Caregiver queries are repository-filtered by current exact Institution/CareGroup scope and reveal only their own roster/Enrollment/Grant-authorized facts; other-Institution existence, counts, stable raw process ids, routes, errors, bodies, stage, and history are hidden. Enrollment/roster creates no stage visibility and Pilot adds no stage dataClass. Current Enrollment uniqueness remains per Institution, different-Institution effects are independent, and an unrelated stage projection change cannot become another Institution's topology conflict. Every render/action owner-rereads; stale family aggregation degrades only the affected episode without replaying old data. C-2f-4-3 must now close the future protocol boundary while preserving the Pilot cross-workspace `NO-GO`; C-2f-5 retains exact result/notification/Handoff. Operational IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4, database apply, ACR publication, secret configuration, capability activation, external traffic, staging, production, and GA remain closed until separate approval.
