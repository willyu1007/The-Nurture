...
- **Updated:** 2026-07-18
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C2e-3 Grant result, recovery, and explicit-empty Handoff convergence after C-2e-2 locked atomic Grant/Thread creation. The active Grant identity is the exact workspace/process/Enrollment/target/canonical-purpose tuple, protected by a raw partial unique index; the fixed directions/data classes form the business profile and any profile change requires replacement. One serializable Nurture transaction uses database transaction time to revalidate and consume the exact submit context, classify all active/history state, create or reuse the exact Enrollment Thread, create the Grant when eligible, and persist CommandExecution/audit refs. Exact replay returns the original effect; a new or racing same-definition command becomes `already_satisfied` without owner or expiry change. Expiry is enforced on every owner read and may be materialized before a fresh new Grant; replacement/revoke/owner-loss remains C-2e-4. Transfer/exit stays C-2f. Guardian/Caregiver operational IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4, database apply, ACR publication, secret configuration, capability activation, external traffic, staging, production, and GA remain closed until separate approval.
