...
- **Updated:** 2026-07-18
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C2f leave/transfer/next-stage convergence after C-2e-4d closed the complete Grant lifecycle/cascade contract. Permanent Grant revoke/replacement/expiry/terminal-owner-loss and source/reply redaction invoke one versioned Nurture-local cascade kernel; Host loss, exact-role suspension, owner outage, and other temporary policy/topology unavailability remain current fences and never write irreversible suppression. Typed `NurtureInteractionContextDependency` rows replace JSON discovery, while `NurtureLifecycleCascadeAudit` stores only root/version/cause/schema/count/hash/closure evidence. One Serializable transaction locks root-first, preflights a hard cap, keyset-loops every dependent without `SKIP LOCKED` or intermediate commit, asserts `NOT EXISTS`, and atomically commits root, contexts, receipts, items/events, attention, Thread projection, audit, and Execution; overflow rolls back before root mutation. Grant invalidation retains protected Messages under author/receiver policy, whereas source and reply redaction keep their distinct cascades. Old immutable replay seeds may still materialize refs-only Host work, but every consumer/provider/open owner-rereads and stops safely. C-2e-4 is complete as a planning contract; C-2f is next. Guardian/Caregiver operational IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4, database apply, ACR publication, secret configuration, capability activation, external traffic, staging, production, and GA remain closed until separate approval.
