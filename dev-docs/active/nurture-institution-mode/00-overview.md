...
- **Updated:** 2026-07-18
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C2f-1 Enrollment pause/resume convergence after C-2f-0 locked the lifecycle and actor boundary. `active|paused` are current Enrollment states, while `ended|withdrawn` are irreversible terminal states and `deleted` is not a Pilot business transition. Guardian and Institution Admin may restrict only their respective family or institution relationship side; Institution Admin may propose a transfer, while Caregiver, Technical Operator, AI interpretation, raw ids, and ambient Host authority cannot change Enrollment topology. A pause is a reversible current fence and cannot invoke the permanent C-2e cascade. Family and institution restrictions must remain independently attributable so neither side can clear the other's pause; the exact hold schema and release rules belong to C-2f-1. Permanent withdrawal, institution end, and completed transfer terminate the old Enrollment and require permanent Grant/work closure. Transfer creates a new Enrollment identity rather than editing `careGroupId`; cross-Institution Enrollments remain independent; cross-workspace matching or automatic Child/Grant/Thread/content migration is forbidden. C-2f-2 through C-2f-5 still own transfer, permanent exit/re-entry, next-stage/cross-workspace, and result/recovery details. Guardian/Caregiver operational IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4, database apply, ACR publication, secret configuration, capability activation, external traffic, staging, production, and GA remain closed until separate approval.
