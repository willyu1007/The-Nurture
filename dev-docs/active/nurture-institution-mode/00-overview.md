...
- **Updated:** 2026-07-18
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C2e-4c Grant-owner loss after C-2e-4b locked voluntary revoke. Only the exact active Grant owner who remains a current exact-Family Guardian may revoke through the common five-minute strong-confirmation contract; another Guardian or institution, caregiver, operator, or service actor cannot substitute. One Serializable transaction uses database time to consume the exact context, change `active -> revoked`, persist server-owned `user_revoked` audit state, invoke the same-transaction dependent fence, and store exactly terminal Grant/Thread refs. Exact replay is immutable, a new eligible revoke against the already revoked Grant is `already_satisfied`, terminal non-revoked states are not mislabeled success, and expected-version races are first-commit-wins. Revocation is irreversible but does not create a permanent family veto: any current Guardian may later complete a fresh Grant authorization for future work only, while old work stays fenced. Revoke is explicit-empty and creates no host activation or delivery effect. C-2e-4c owner loss and C-2e-4d cascade closure remain open. Guardian/Caregiver operational IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4, database apply, ACR publication, secret configuration, capability activation, external traffic, staging, production, and GA remain closed until separate approval.
