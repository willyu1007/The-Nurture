...
- **Updated:** 2026-07-18
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C2e-4b owner-initiated Grant revoke after C-2e-4a locked replacement. Replacement is an owner-only, newly reviewed authorization rather than in-place edit or renewal: one Serializable transaction consumes an exact five-minute context, marks the active expected-version old Grant `replaced`, creates a new active Grant with the same owner and single `supersedesGrantId` lineage, reuses the exact Enrollment Thread, fences old-Grant work, and stores exact old/new/Thread refs. `replacedAt` and new `effectiveFrom` share database transaction time; no active overlap or gap exists. Same-definition is `already_satisfied`, terminal old Grants cannot replace, topology transfer stays C-2f, and Thread reuse never reauthorizes old content. Replacement is explicit-empty and creates no host activation/content/delivery effect. C-2e-4b revoke, C-2e-4c owner loss, and C-2e-4d cascade closure remain open. Guardian/Caregiver operational IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4, database apply, ACR publication, secret configuration, capability activation, external traffic, staging, production, and GA remain closed until separate approval.
