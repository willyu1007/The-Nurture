...
- **Updated:** 2026-07-18
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C2f-3b atomic terminal-closure convergence after C-2f-3a locked permanent-exit actions, actors, statuses, and fixed reasons. Guardian Chat/board/workbench add `withdraw_family_enrollment -> nurture.family_care.withdraw_enrollment`, while Institution workbench retains `close_enrollment -> nurture.institution.close_enrollment`; Institution board remains read-only. Any current exact-Family Guardian may independently withdraw the shared Enrollment without Grant-owner, first-Guardian, primary-Guardian, or unanimous Co-Guardian authority, and any current exact-scope Institution Admin may end service without family countersignature. Both actions require five-minute strong confirmation, accept no client reason/audit fields, and map family withdrawal to `withdrawn + family_withdrawn` and institution end to `ended + institution_service_ended`; transfer remains `ended + care_group_transfer`. Current `active|paused` may enter a terminal path. Active holds cannot veto terminal exit and must later close as system consequences rather than be misrepresented as side releases. Terminal Enrollment never resumes; C-2f-3b owns exact hold/Grant/Thread/work transaction closure, C-2f-3c owns fresh re-entry identities/history, and C-2f-5 owns exact results, notification, and Handoff. C-2f-4/C-2f-5, operational IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4, database apply, ACR publication, secret configuration, capability activation, external traffic, staging, production, and GA remain closed until separate approval.
