...
- **Updated:** 2026-07-19
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C3-0c-2 semantic blocks/actions after C-3-0c-1 locked the subject-provider wire. Discovery and current revalidation are separate `list_subject_contexts` and `resolve_subject_context` operations. The wire uses a new workspace/scenario/principal-bound opaque `ScenarioSubjectContextRefV1`, never a raw `DomainContextRef`; a collection is one `subject_collection` context and the provider never expands members. Results are closed `resolved|needs_selection|context_changed|unavailable` variants with an exact privacy allowlist. Context refs expire within 30 minutes, cursors within 5 minutes, and pages contain at most 20 candidates. A subject context remains a locator, not relationship proof, action authority, replay evidence, cross-surface permission, or offline entitlement. C-3-0c-2 retains the exact safe-label encoding and semantic block/action vocabulary; C-3-0c-3 retains renderer/persistence classes; C-3-0c-4 retains conformance/adoption. This checkpoint is planning-only: no contract, manifest, source, schema, migration, route, UI, runtime, cache, secret, capability, provider, database, or traffic changed. C-3-0c-2 through C-3-0e, Guardian/Caregiver IIB, C-4 Institution IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4 and all external enablement remain separately unauthorized.
