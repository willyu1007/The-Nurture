...
- **Updated:** 2026-07-18
- **Owner:** willyu1007
- **Next step:** Continue Pilot-0-C2e-4d dependent cascade closure after C-2e-4c locked Grant-owner loss. Every Pilot Grant must bind both the exact owner Participant and the exact Guardian RoleAssignment through additive `grantedByRoleAssignmentId`; a later RoleAssignment for the same Participant cannot revive the old Grant. Host account/workspace loss denies only that canonical user's Host access and does not mutate Nurture facts. Exact RoleAssignment suspension blocks the Grant without terminalizing it and may resume only on the same role row; exact role revoke/expiry/delete/effective end permanently invalidates the Grant with no ownership transfer. Self-exit composes role termination, owner-Grant revoke, fence, and one Execution atomically. Other terminal loss fails closed immediately and converges through idempotent Nurture-local reconciliation. A different current Guardian may later complete full confirmation for a new independent future-only Grant; the transaction may terminalize an unreconciled old Grant, reuses Thread only as container, emits new Grant/Thread refs, and never sets replacement lineage or revives old work. Owner-loss paths remain explicit-empty and create no Host delivery effect. C-2e-4d cascade closure is next. Guardian/Caregiver operational IIB, topology/operations, and final Go/No-Go remain open. Pilot-1 through Pilot-4, database apply, ACR publication, secret configuration, capability activation, external traffic, staging, production, and GA remain closed until separate approval.
