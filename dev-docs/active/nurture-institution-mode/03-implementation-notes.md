...
- 2026-07-15: The Nurture delivery PR exposed a clean-run bootstrap defect after the X5 implementation gate had passed locally: the pinned `templates/web-workbench` package owns a standalone lockfile but is intentionally outside the My-Workflow-Base root workspace, so plain `pnpm --dir ... install` walked upward and installed only the five root-workspace projects. The delivery fix adds `--ignore-workspace` to the repository preparation script and all four CI jobs, preserving the template as an independently reproducible package and keeping local/CI behavior identical. A fresh `/tmp` checkout installed 26 standalone packages and built successfully; the exact merged My-Chat pin and all source hashes remained unchanged.
