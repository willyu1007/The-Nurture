...
- 2026-08-05: Reused T-002 and froze only the `C30-I1-D` neutral Base
  ...
  prepare/submit, direct result, claimed Step bind/recovery and effect-identity
  inputs as D1-D5; normalizes the shared driver to
  `scenario_direct_empty_v1|workflow_claimed_step_v1`; and adds `scenario_key` to
  direct effect identity. Standalone I1-D wire/codec/schema/neutral conformance is
  distinct from I1-F manifest dependency, legacy/vNext and source-identity
  convergence. No Base/My-Chat/Nurture source, build, schema/database, runtime,
  capability, deployment, activation, T-008, Pilot or traffic action ran. I1-D1
...
- 2026-07-31: Repaired the 21 previously reported T-002 documentation warnings.
  ...
  boundaries, instead of accumulating unrelated pronouns across an entire
  multi-thousand-line specification. Remaining genuinely dense references were
  replaced with explicit contract, operation, candidate and effect names. Strict
  document/anchor lint now passes the 31-file T-002 bundle and the 372-file
  repository scan with 0 errors and 0 warnings. The cleanup changes diagnostics
  and wording only; no G1-G7 decision, task
  dependency, acceptance gate, product source/schema/runtime, environment or
  traffic state changed.

- 2026-07-31: Accepted and documented the full G7 bounded internal Pilot structure
  as G7-0 entry/authority freeze, G7-A Pilot-2 activation/bootstrap, G7-B Pilot-3
  ordered rehearsal/terminal disable, G7-C fresh-baseline 120-hour observation and
  G7-D terminal evidence/recommendation. The plan preserves separate stage, release,
  fault-plan, evidence and review authorities；requires a new Pilot-4 authorization
  and row after the Pilot-2 row is permanently closed；uses one exact seven-account
  synthetic cohort and seven planned paths；and makes `pass|no_pass|stopped`
  disjoint with no limited PASS or automatic next-scope authority. Mainline
  successor development may continue but cannot mutate the observed Pilot
  environment. Future execution is proposed as
  `nurture-bounded-pilot-observation` plus a My-Chat companion；no task/Feature,
  source/schema/migration, database/cloud, secret/KMS, artifact/deployment,
  capability, row, activation, observation window or traffic was created or
  changed.

- 2026-07-31: Accepted and documented the full G6 scope, internal ordering and
  acceptance boundary. G6-A consumes rather than duplicates T-004～T-007 product
  handoffs; A1/A2 close current C-3 then C-4, A3 prepares D sources/operations,
  A4 assembles the complete candidate and A5 seals disposable evidence/TR census.
  G6-C/D now have explicit publication/provision/migration/deploy/readback and
  restore/hard-stop/custody/readiness order. Drift is split into G5-shared,
  complete-Pilot-only and evidence-only layers so Host/topology changes do not cause
  unnecessary dual-platform reruns. G6 has no generic limited PASS and exits only
  default-off with Pilot-2 authorization pending. T-008 G5-0 now carries a read-only
  Pilot carry-forward census. The G6 scope update is documentation/governance
  only；no new task,
  feature, source/schema/migration, cloud/database, artifact/ACR, secret/KMS,
  capability, row, deployment, activation or traffic state was created or changed.

- 2026-07-30: Accepted the Stage G6 top-level planning overlay as five packages:
  G6-0 Candidate/Evidence Reconciliation, G6-A C3/C4/D Implementation Closure,
  G6-B Pilot-0-E, G6-C Pilot-1 private publication/default-off deployment, and
  G6-D default-off qualification/G7 handoff. The plan makes G6-0, the
  C3->C4 qualification chain, candidate/D-seal join, E, Pilot-1 authorization,
  deployment, and final readiness join serial. D non-stateful preparation may
  overlap late C3/C4 work, and owner-separated Pilot-1 provisioning may overlap
  only after E Go plus separate authorization. It also prevents G5
  `NurtureServiceCandidateV1` from being treated as the complete Pilot candidate:
  Candidate-defining G6 changes require a successor Service Candidate and
  affected G5 revalidation. This was a documentation/governance decision only;
  no implementation task was registered and no source, schema, migration,
  disposable/cloud environment, artifact/ACR, secret/KMS, capability, row,
  deployment, activation, or traffic was authorized or changed.

- 2026-07-29: Completed Nurture publish-cycle P3 and repository cleanup. My-Chat `5f52327` repaired the canonical-ref zero-version codec and `a4768fe` repaired deterministic completion output ordering; Base `5c04dce` remains contract-identical to `63d47d2`. Nurture re-pinned the exact 161-file My-Chat X5 population and passed the real disposable two-database X5 journey. The consolidation was merged to `main` at `ab92fde`; exact main CI `30412303062` passes all seven jobs. The fully merged local/remote integration branch and all auxiliary Nurture worktrees were deleted, leaving one clean local/remote `main`. The P3 result closes the Nurture leg of `C30-I0-C`, not the three-repository gate: Base/My-Chat branch/worktree disposition and `C30-I0-D` false/empty immutable-baseline proof remain before `C30-I1`.

- 2026-07-29: Created `codex/T-002-nurture-consolidation` from clean `main`, committed the pre-existing task-continuity tooling separately, fully merged `codex/t002-cross-repo-acceptance`, and selectively harvested only the fail-closed environment repair from `codex/nurture-env-convergence`. The stale scenario-platform and detached slice worktrees remain excluded donors. Frozen install, exact three-repository pin verification, strict context/environment/governance/docs, full typecheck, frontend lint, 187 unit tests, routing and persistence boundaries, N1/X4 contracts, five production plus one dev-host migrations, production boundary and 37 DB tests, and dev-host boundary and 19 tests all pass. A real exact-revision two-database X5 rerun then found a cross-owner semantic defect: Base canonical-ref v1 accepts non-negative versions and Nurture preserves new records at version `0`, while My-Chat `30792cd` still rejects zero in the Handoff value codec. The Host consequently completes the Step as `manual_review_required` with `workflow_handoff_invalid_draft` and no materialized Handoff. No Nurture-side lossy workaround was applied, no persistent database was used, and promotion to `main` is held pending the My-Chat owner fix, re-pin, and clean X5 rerun.
...
- 2026-07-21: Completed the read-only `C30-I0-A` cross-repository baseline census and recorded it in `artifacts/12-c30-i0-baseline-inventory.md`. The canonical Base revision `acba4e7` is clean; My-Chat `db22de6` is one commit ahead of origin but its primary worktree is occupied by unrelated cloud-deployment changes; the Nurture T-002 branch remains an uncommitted C/D decision corpus mixed with a separate untracked T-003 UIUX task. All three T-029 `scenario-platform-convergence` worktrees are based on the observed canonical HEADs but contain only uncommitted candidate changes. The Nurture T-029 worktree overlaps ten current T-002 paths and stores direct platform Child/Family refs on local profile rows instead of the locked scenario-global typed anchors plus workspace associations. T-029's neutral refs, command/event/receipt, inbox/outbox, registry, integration-lock and recovery code remain donor candidates, not accepted adoption. Its umbrella capability/source shape, mutable local package evidence, non-hermetic installed verifier, direct-ref schema, and edits to older T-002 docs require rework or exclusion. Canonical Base/Nurture/My-Chat typechecks and tests remain healthy; no DB apply/query, source/schema/runtime/gate/environment/artifact/secret/provider/traffic change was performed. At the I0-A checkpoint B/C/D were pending and C30-I1 was NO-GO; the later I0-B entry above supersedes only B's pending state.

- 2026-07-21: The Pilot-0-D whole-contract quality review found and repaired four cross-section defects before implementation. Pilot-3 now has one exact monotonic terminal lineage—`gates_closed -> final_binding_bound -> plan consumed_success -> Pilot-2 stage authority consumed -> terminal seal`—whose valid partial prefix may append/retrieve only the next successor; consumed plan/stage heads prove only stage consumption/sealing, and completed historical heads are verification-only. Traffic readiness now uses stable `TR-P0-1..6` and `TR-P1-1|2|3a|3b` ids through `pilot0_traffic_readiness_census_v1`; E requires all P0 and P1-1/2/3b closed and permits only native/external delivery P1-3a as `accepted_scope_exclusion`. The canonical `externalProductTrafficCount` reconciles public My-Chat ingress with zero external product traffic by counting non-allowlisted admitted product/owner/business/delivery effects while excluding only audited pre-admission scans, health, and allowlisted control/service/evidence traffic. Any admitted eighth/unplanned Nurture question or business effect now makes Pilot-4 `no_pass`; only pre-admission zero-effect rejection remains a negative probe. Architecture, schema ownership, conformance, readiness, notes, pitfalls, and verification projections were synchronized; no source, schema, migration, runtime, database, cloud, artifact, secret, gate, provider, or traffic effect was authorized.

- 2026-07-21: Final Pilot-0-D rereview repaired authority and recovery gaps that the initial closure pass had missed. The complete candidate now binds the exact topology source, canonical static behavior configuration, and every controller/resolver revision while excluding live gate values; a separately authorized disposable dual-owner evidence run produces `pilot0_d_predeployment_evidence_seal_v1` before E. E, deployment, stage authorization, prerequisite seals, activation row, daily observation seals, and terminal review each have distinct signed/current identities. Real Pilot C-0 bootstrap now has a non-aliasing `pilot0_institution_provisioning_spec_v1`, exact claim/workload/audience, one-effect Host operation, writer-fenced `committed|confirmed_no_effect|unknown` recovery, permanent close, and rollback custody. Host admission rereads current authorities before each owner attempt; Nurture first verifies the fresh signed private invocation/nonce and then the persisted bounded admission locally, never making a remote gate read inside its transaction. The first row remains bootstrap-only until exact Host `owner_committed + spec consumed + quarantine clear`, so response loss cannot expose ordinary routes. Pilot-3 success now requires a monotonic `consumed_success` plan successor, and every non-PASS Pilot-4 window has terminal-stop evidence that may be sealed in the bounded evidence-only grace. The hard stop closes every ordinary ingress/private/claim/worker/send/open seam while preserving only isolated recovery. Restore now defines a monotonic external privacy-ledger protocol, distinguishes Grant revoke from erasure, converges restored rows through owner commands, and measures RTO through safe sealed completion. Pilot-2 readiness, Pilot-3 terminal rehearsal, Pilot-4 baseline, five daily segments, exact seven planned question paths, and independent terminal review are separate evidence stages. Lower-level status, responsive-Web, role/surface, sample, telemetry, and stage wording were synchronized without code, schema, database, cloud, artifact, secret, gate, or traffic effects.

- 2026-07-21: Locked Pilot-0-D D-0 through D-7 after independent product/operations, release-governance, and security/reliability assessment plus cumulative repair. The design separates `complete_pilot_candidate_recipe_v1`, immutable undeployed `complete_pilot_candidate_id`, body/secret-free D evidence, `pilot0_e_release_decision_v1`, and later `pilot_deployment_binding_v1`, removing both the E-before-deployment cycle and the Pilot-3 kill-switch/Pilot-4 re-enable cycle. The selected internal topology is a dedicated Alibaba Cloud Hangzhou `pilot` VPC with separate My-Chat/private-Nurture ECS/Compose hosts and RDS owner boundaries, My-Chat-only Redis/public ingress, a production Nurture owner API plus retention/erasure worker, private Alibaba ACR publication only in Pilot-1, and Alibaba Secrets Manager/KMS with no static bearer fallback. Responsive Web Chat/board/workbench and controlled in-app Notification/deep link are in; native mobile, OS push/SMS/email, external recipients/provider, protected AI and real data are out. D locks two independent technical gates, current qualification/E/deployment reread, disable-only Technical Operator, separate operational credentials/RACI, body-free telemetry and `POPS-SEV0/1/2`, independent encrypted PITR at `RPO <= 15 minutes / RTO <= 4 hours`, externally copied Nurture erasure ledger, forward-only rollback, 30/365-day custody, a fresh post-rehearsal activation row, uninterrupted 120-hour observation, and exactly seven planned question round trips across the three child scopes. The initial D review inventory of six P1/three P2 decision ambiguities is closed at `DR-P0=0 / DR-P1=0 / DR-P2=0`; six P0 and three P1 readiness items remain open, native/external delivery P1-3a is an accepted scope exclusion, no candidate/QR exists, and no code/schema/migration/artifact/ACR/DB/secret/environment/gate/traffic action was authorized.

- 2026-07-20: Repaired the Pilot-0-C clarity and governance findings without changing product code or runtime authorization. Added `10-pilot0-c-current-decision-index.md` as the concise status/precedence entrypoint and reclassified `09-pilot-readiness.md` as the detailed decision ledger. Current severity namespaces are now disjoint: `DR-*` for design-review defects, `TR-*` for traffic-readiness blockers, and `QR-*` for immutable-candidate qualification findings. The identity contract now states one normative two-owner opaque-binding chain: My-Chat raw Child/Family ids remain in My-Chat scenario-binding rows; typed owner refs point to Nurture scenario-global anchors; workspace associations connect anchors to local Child/Process/child-scoped Family. The context and schema projections explicitly forbid raw platform ids in Nurture while defining the minimum My-Chat owner API responsibilities and making concrete type/port shape a mandatory C30-I1/I2 implementation deliverable rather than an unresolved product decision. A final semantic pass also removed the contradictory blanket ban on anchors in “evidence”: typed anchors are allowlisted only in My-Chat binding `ownerRef`, Nurture anchor/association persistence, the short-lived private signed current-owner envelope, and authenticated reconciliation, while every client/delivery/telemetry/qualification destination remains forbidden. Overview, plan, roadmap, test projection, verification, and pitfalls were synchronized. Pilot-0-C remains decision-complete, implementation-open, and external-traffic NO-GO; no C30/C40 node, schema, migration, route, runtime, database, secret, capability, allowlist, provider, or traffic is authorized by the clarity repair.

- 2026-07-20: Reopened and repaired the C-3/C-4 design after My-Chat revision `db22de66c2e58fec5e24be0150458b36c02e9682` introduced protected platform Child/Family schema semantics. The prior “My-Chat has no canonical Child/Family” and direct `myChatChildId -> workspace-local NurtureChild` assumptions were invalid. The replacement model keeps My-Chat as owner of platform Child/Family, stewardship/membership, `FamilyChildMembership`, scenario bindings, and the durable identity operation; Nurture owns body/PII/authority-free typed anchors, exact workspace associations, ChildCareProcess, child-scoped Family, roles, Enrollment, Grant, content, and policy. Binding resolution now explicitly supports both-reuse, Family-reuse/new-Child, Child-reuse/new-Family, and both-new, with conflicting heads quarantined; the Host operation separates terminal state from `outcome_unknown`, Nurture provides a strict privacy-closed writer-fenced status lookup, empty bound anchors are unique reusable endpoints, and association graph constraints are exact. Institution intake remains Roster/Intent only; Co-Guardian membership and role are a recoverable two-owner saga; platform fields are not copied into Nurture. Exact platform identity reuse is allowed across workspaces/scenarios, while local dossier/authority portability, PII matching, automatic merge/rebind, and cross-workspace discovery remain disabled. C-3 adds separately pinned `platform_child_family_identity_source_v1`; `db22de6` is schema-only observed input and does not replace the historical My-Chat workflow pin or satisfy adoption. Product, domain/privacy, and cross-repository final rereviews returned `DR-P0=0 / DR-P1=0 / DR-P2=0` and all design gates pass; C-4/Pilot-0-C is decision-complete and implementation-open, with implementation separately unauthorized and Pilot-0-D design discussion next. The C-3/C-4 repair changes documentation/context plus the exact Nurture scenario content hash only; no contract package/product source, Prisma schema/migration, API/runtime, database, capability, or traffic changed.

- 2026-07-20: Final C-4 review repair pass removed the remaining cross-stage dual tracks. RosterEntry now has one canonical `active|linked|closed` status plus typed terminal reason, explicit 30-day unverified-intake erasure, linked-prefill clearing, and 365-day safe-label de-identification, superseding C2f's historical transferred/withdrawn/ended status shorthand. Pre-D JI1 uses only a disposable controller-owned synthetic spec and a single-business-effect `c4_bootstrap_evidence_authorization_v1`; Host claims a deterministic operation and converges owner response loss through an isolated status lane whose `confirmed_no_effect` result requires the C0 writer fence and fence-protected CommandExecution absence. Real Pilot provisioning remains D-owned. C-4 evidence alone uses `nurture_institution_composite_evidence_v1`; the complete-Pilot profile is separately locked by D and admission independently requires current C-3 qualification, C-4 qualification, E decision, and Pilot-1 deployment. C-4 adoption names and hashes thirteen source sets. Every nonempty relationship/status effect freezes one owner-owned recipient cohort in one draft and materializes at most one Handoff; My-Chat alone fans that Handoff out into per-recipient candidates. Staff Invitation supports new-member and reauthenticated existing-member branches; Pilot staffing enforces one human Caregiver/Lead pair across the Workspace cohort, with transactional elapsed role/Lead normalization. C-4 active-source-only transfer rules supersede the historical C2f note that paused/archived source Groups could transfer out: proposal, review, open, confirm, and decline all deny there, while cancel/cleanup remains narrow. The staffing close extension owns the exact no-reply resolved graph, privacy-after-close convergence, and a strict send/open fence over active Group/no Holds/current effective Grant. JI1 performs invitation acceptance before bootstrap from a blank fixture; every later journey executes its namespace-local setup prefix anew with fresh authorization, operation, Execution, and business identities, and JI5 keeps the source Group active while using non-overlapping T1 role episodes. These are design/evidence corrections only; no source, schema, migration, runtime, authority row, environment, capability, provider, or traffic changed.

- 2026-07-20: Locked Pilot-0-C4-5 and completed the C-4/Pilot-0-C design contract. Future implementation enters only with separate authorization and current `C3_QUALIFIED_DEFAULT_OFF`, then runs strict `C40 Base -> C41 My-Chat -> C42 Nurture -> C43 composite/producers -> C44 evidence -> C45 qualification`. C-4 extension and composite candidates are immutable/non-circular; evidence authorization/results, qualification, D/E, deployment, activation, and environment stay outside identity. L0–L7 and JI1–JI8 cover exact topology, staff, three families, Holds, transfer, service-close/re-entry, staffing closure, disable/rollback, authenticated rendering, three fresh fault/privacy runs, and teardown. Qualification uses its own pre-seal, deterministic signed verifying genesis, envelope, terminal event, and current resolver. Only `C4_QUALIFIED_DEFAULT_OFF / PILOT0_D_PENDING / EXTERNAL_TRAFFIC_NO_GO` may succeed. Pilot-0-D discussion is next; no C40–C45 node, candidate, store/event, source, schema, runtime, database, environment, secret, capability, provider, or traffic changed.

- 2026-07-20: Locked Pilot-0-C4-4a-d. Staffing review is derived from one acknowledged/unreplied Item, active Attention, matching typed claimant/assignment, and terminal exact Caregiver role; no second aggregate or takeover queue is added, and temporary fences never derive closure. Exact Admin workbench action `close_staffing_blocked_family_care_item` always uses an original claimed Step and atomically writes Item `closed + claimant_role_ended_unfulfilled`, one closed Event, resolved Attention, Execution, and status seed while leaving the source Receipt acknowledged and creating no reply Message, family Receipt, care fact, Grant/Thread mutation, or reassignment. Operational `closed` stays distinct from privacy `suppressed`. A separate `family_care_status_attention/family_care_operational_closure` targets commit-time current Guardian roles at `family_board/recent`, uses generic id-only copy, and composes with the existing C-3 unknown-safe terminal presenter without widening either C-3 Handoff/candidate. Group pause permits effect-decreasing closure; technical disable blocks new work and never backfills. C-4-5 is next. No contract, source, schema, runtime, Item/Event/Attention/Step/Handoff/Notification row, database, environment, capability, provider, secret, or traffic changed.

- 2026-07-20: Locked Pilot-0-C4-3a-d without adding a second Enrollment lifecycle. Institution Hold/release and transfer cancel are C2f direct-empty actions; transfer proposal and service close always use the original durable claimed Step because they may create the separately typed body-free `guardian_relationship_attention`. Proposal snapshots one cohort-level `review_enrollment_transfer` draft and service close snapshots one cohort-level `enrollment_relationship_changed` draft, each containing the exact commit-time Guardian RoleAssignment set; zero recipients stores `[]`, a nonempty business effect materializes at most one Handoff, same-Step replay is immutable, and late/new role episodes are not backfilled. My-Chat derives per-recipient candidates under that one Handoff. Transfer confirmation remains C-3 and creates no second Handoff. Service close maps only to `ended + institution_service_ended` and commits the complete Enrollment-rooted zero-survivor closure independently of Notification delivery. Notification id is the only external locator; open requires the exact current recipient role and rereads current transfer or retained terminal history without requiring an active Enrollment/Grant. Re-entry remains C-4-2 invitation plus C-3 confirmation, never a notification action. C-4-4 is next. No contract, source, schema, runtime, Step/Handoff/Notification row, database, environment, capability, provider, secret, or traffic changed.

- 2026-07-20: Locked Pilot-0-C4-2a-d. RosterEntry is now an institution-local `active|linked|closed` intake record with only bounded unverified prefill; the RosterEntry is never child/profile/family authority, has no singular Host invitation pointer, and links only inside the C-3 Enrollment transaction. `initiate_enrollment`, cancel, and reissue use the existing identity-invitation coordination lane: non-deliverable Host shell -> Nurture seven-day EnrollmentInvitation intent/Execution -> same shell delivery activation -> exact Host acceptance/membership -> signed C-3 prospective continuation. Host acceptance does not consume the intent or create Participant/Guardian/Enrollment authority; family profile establishment remains independent, exact family confirmation atomically consumes intent/context and links Enrollment/Roster, and Grant/Thread remain separate. Cancel/decline/reissue/consume are first-commit-wins. Terminal same-Institution re-entry uses fresh roster/invitation/Enrollment/Grant/Thread identities bound to the exact terminal predecessor leaf. C-4-3 is next. No source, schema, runtime, invitation row, database, environment, capability, provider, secret, or traffic changed.

- 2026-07-20: Locked Pilot-0-C4-1a-d after product/domain/cross-repository review. The active Pilot subset is typed Institution display update; CareGroup create/update/pause/resume/zero-dependent archive; immutable Institution policy revision plus group binding; exact Caregiver assignment; exact-role-bound Lead designation; and Caregiver revoke. C0 remains the sole Institution/first-Admin create path. Staff onboarding uses the separate signed Host invitation saga: non-deliverable contact shell -> Nurture seven-day StaffInvitation intent/Execution -> shell delivery activation -> exact Host acceptance/membership -> private callback Participant bind/intent consume -> separate Admin role and Lead actions. No Step/Handoff/domain-driver identity shortcut exists. Lead is a distinct `lead_caregiver + care_group` RoleAssignment with typed self-FK to the exact same-user/same-group operational Caregiver role; Lead alone grants no access, one current Lead is required for readiness, and revoke terminalizes both episodes without blocking on old acknowledged work. Group pause is a reversible zero-cascade business fence; close maps to archived only after every current dependent is absent. The review also demoted the old class-of-ten/two-Caregiver/role-overlap fixture to non-Pilot scale evidence. C-4-2 roster/Enrollment Invitation is next. No contract, package, source, manifest, schema, migration, policy row, invitation shell/intent, route, presenter, action, runtime, candidate, database, environment, capability, allowlist, provider, secret, or traffic changed.

- 2026-07-20: Locked Pilot-0-C4-0 after independent product, domain-security, and cross-repository reviews. The Institution actor path is exact current Host membership -> Participant -> `institution_admin + scopeType=institution` -> active Institution; `institution_board` is current/read-only and `institution_workbench` is current/history plus the sole Institution write surface, with no Institution Chat or legacy Caregiver-route fallback. The review found that direct C-4 additions to the C-3 monolith would invalidate the qualified C-3 absence/hash evidence, so C-4 now requires an immutable content-addressed C-3 component and a separate `nurture.institution.iib.v1` fragment, migration ledger, extension candidate, composite candidate, composition recipe, pre-E evidence authority, and qualification chain. The forward authority graph keeps C3 -> C4 -> D candidate -> E decision -> Pilot-1 deployment -> Pilot-2 activation non-circular. Base remains neutral, My-Chat owns identity/shell/runtime/composition/evidence stores, and Nurture owns Institution facts/resolution/presentation/actions. C0 remains the only first-Institution bootstrap; invitation coordination stays outside domain-action drivers/Handoff identity transport; staffing closure and relationship-attention producers remain mandatory later C-4 decisions. C-4-1 topology/policy/staff/Lead lifecycle is next. No contract, source, package, manifest, schema, migration, route, presenter, action, runtime, candidate, controller, database, environment, capability, allowlist, provider, secret, or traffic changed.

- 2026-07-20: Completed the repeated C-3 repair/review loop. Independent product, security, and governance reviewers each returned PASS with `DR-P0=0 / DR-P1=0 / DR-P2=0` after verifying the exact Caregiver scope/claimant fence, typed actor, transport-denial boundary, evidence-run authorization, qualification genesis/recovery, product surfaces, privacy, fixtures, and strict DAG. Pilot-0-C4 is therefore entered only as `DESIGN DISCUSSION OPEN / IMPLEMENTATION UNAUTHORIZED`; C-4-0 Institution IIB scope/owner boundary is the next open decision, and no C-4 design choice or implementation/adoption is claimed.

- 2026-07-20: A second three-way C-3 re-review found and repaired the remaining executable dual tracks. Activated C-3 policy now ignores ThreadParticipant as authority, requires exact `caregiver + scopeType=care_group`, binds acknowledged/replied protected reads and family reply to the same current claimant role episode, and denies Institution/Enrollment-scoped roles plus Admin/Lead substitution. `business_actor_ref` is legacy-only; activated rows require typed Participant actor fields plus separately named Host-principal provenance, with service/recovery/operator identities excluded. The recovery endpoint now rejects signer/codec/nonce/frozen-binding failures at transport with zero status/fence/application call; protocol `unknown` is reserved for an authenticated exact-bound lookup that cannot classify compatible evidence. Disposable `c3_component_v1` evidence uses the explicit Base-owned `c3_evidence_run_authorization_v1`, signed by the isolated evidence controller and resolved from an append-only fail-closed My-Chat authority store; `complete_pilot_v1` alone requires current qualification/E resolvers. Qualification now executes non-circularly as I0–I9 pre-seal, deterministic signed `verifying` genesis CAS, controller-signed envelope and terminal-child CAS, then resolver-confirmed I10 exit; crash after genesis is idempotently recoverable. Enrollment decline may follow an independently committed first-Guardian/profile without deleting family facts; the qualifying fixture path now explicitly includes Enrollment confirmation before Grant/Thread. Old surface aliases, clarification/follow-up, body-derived AI assistance, and broad class-inbox actions are marked non-activatable future/legacy material. At the intermediate checkpoint C-4 still awaited repeated PASS；the later same-day PASS entry records completion and supersedes the pending status.

- 2026-07-20: Repaired the complete C-3 decision set after an independent logic/clarity review identified six implementation-blocking ambiguities. The repair makes `09-pilot-readiness.md` the decision SSOT and keeps architecture/schema/test documents as projections. C-3 now registers exactly four Guardian/Caregiver Nurture product surfaces; Institution keys remain absent/negative until C-4, while neutral Host fixtures alone exercise all three renderer families. The protected zero-copy guarantee starts only at the registered composer; ordinary user-authored Chat remains under My-Chat policy and is never automatically promoted. R7 now yields only `open|acknowledged` current family-care candidates and treats `replied|suppressed` as recent/history terminal state. The implementation DAG is strict `C30 -> C31 -> C32 -> C33 -> C34 -> C35`, with C30 as the sole shared baseline owner. Disabled response-loss classification now has a dedicated `my-chat-execution-recovery` caller/issuer/audience/endpoint/key/verifier lane that cannot resolve business authority, execute a command, or disclose content. Qualification now uses an isolated `c3_qualification_controller`, append-only signed predecessor chain, content-addressed evidence store, and fail-closed current-state resolver; activation consumes the resolver rather than a cached label. Candidate and provider `skipped` values are separately namespaced; J1 proves two recipient rows from one Handoff; setup fixtures stop before accepted membership/Participant/Guardian/Enrollment/Grant/Thread. C-4 must later close the terminal-caregiver staffing-review workflow. The clarification entry supersedes earlier shorthand that referred to six C-3 Nurture surfaces, an undifferentiated “activation record,” ordinary Chat as private draft material, overlapping C30/C31 implementation, or unsigned qualification lifecycle. No code, schema, migration, route, capability, database, secret, environment, or traffic changed. At the intermediate checkpoint C-4 entry remained gated；the final same-day PASS entry later in the log supersedes that pending status.

- 2026-07-20: Locked Pilot-0-C3-5-0 through C-3-5f after independent product-surface, domain-policy/privacy, and cross-repository adoption reviews plus a backward check against B3-4, C-2, C-3-0..4, C-4, and Pilot-0-D/E. The later same-day consistency-repair entry supersedes the trust-path, DAG, surface, fixture, qualification shorthand, and next-step status in the historical lock note. C-3-5 means cumulative default-off implementation and qualification, not actual activation. `C30-I0..I4` make the earlier shared-baseline implementation machine-gateable; C35-I0..I10 converge Base/My-Chat/Nurture code and the activation kernel before one dependency freeze and artifact assembly. The review removed a candidate/evidence hash cycle: immutable `component_candidate_id` binds build/configuration inputs only, while every result binds that id and a later signed `qualification_envelope_id` separately binds the evidence-index digest, final false/empty census, and result without modifying candidate-pinned source commits. Positive-only My-Chat `ScenarioWorkspaceActivation` uses typed release-candidate and deployment identities: disposable C-3 proof binds the component/disposable deployment, while a later Pilot-2 row must bind the C-4+D complete candidate referenced by E plus Pilot-1's exact deployed artifact/config/KMS/signing binding. Base `scenario_activation_admission_source_v1` is the single SSOT for admission and exact execution-status lookup; candidate evidence separately pins source, My-Chat persistence/issuer, and Nurture verifier/fenced resolver revisions. Cross-database kill semantics keep Host admission, the pre-owner-call C-3-0b transport nonce, and the Nurture `CommandExecution + business effect` commit distinct. Admission expiry blocks uncommitted work, while committed same-Step/direct recovery uses the old hash only as provenance. Fenced `committed|confirmed_no_effect|unknown` lookup prevents guessing when an admission produced no Execution; unknown stays quarantined, no-effect closes claimed/direct work without Handoff, and committed claimed work alone enters the five Handoff cases including explicit-empty `[]`. Frozen provenance prevents newer runtime reinterpretation. Exact J1-J4 surface/view paths use `recent` after reply, Notification-id-only native-to-web requires explicit same-user Workspace switch, capture-time masking is mandatory, and the C-4 fixture boundary is narrow. J1-J4 remain the communication/Caregiver strand; a separate C-3-2 Guardian relationship/authority strand creates qualifying Grant/Thread facts through real C-3 paths. A disposable `evidence_release_controller` is destroyed afterward; Pilot-2's distinct controller remains the sole Pilot enable authority. Successful future implementation may report only `C3_QUALIFIED_DEFAULT_OFF / C4_PENDING / EXTERNAL_TRAFFIC_NO_GO`; C-3-5 ends with the environment activation bundle false and active Workspace rows empty. Current repositories have none of the C-3 implementation/candidate/gate evidence and remain readiness NO-GO. At that historical checkpoint C-3 and C-3-5 were `DESIGN COMPLETE / IMPLEMENTATION OPEN`; the final same-day PASS entry later in the log now opens C-4 design discussion while keeping C-4 implementation unauthorized. No contract, package, source, manifest, candidate, schema, migration, KMS/signing material, route, UI, runtime, artifact, evidence run, gate row, capability, allowlist, database, provider, secret, environment, or traffic changed.

- 2026-07-20: Locked Pilot-0-C3-4-0 through C-3-4f after consolidated product-surface, domain-policy/privacy, and cross-repository adoption reviews plus a backward check against B3-2 and C-3-0..3. Ordinary continuity keeps the existing presentation/action/protected authorities: product route plus `current|recent|history`, exact `stay|discard_and_navigate`, original Execution/Step replay, and destination current owner presentation. The new `scenario_notification_continuity_source_v1` and atomic default-off `scenario_notification_continuity_v1` cover only recipient-bound Notification continuity. My-Chat will persist an immutable per-commit-candidate `pending|materialized|skipped` plan, use a typed workspace/recipient/Handoff source link rather than client-visible generic targets, distinguish legacy and vNext rows with an explicit closed source-kind discriminator, and settle the Handoff only after every candidate is terminal. The typed link freezes recipient binding, non-sliding horizon, contract/allowlist/copy/intent hashes but no route/view; current open alone selects an allowlisted destination. DTO/provider/deep link remain Notification-id-only, create/send use a signed non-human owner-read caller with independent credential/key/nonce evidence, provider work uses lease/reclaim/backoff/dead-letter, and interactive open binds the exact historical recipient role episode before issuing a destination-bound hash-only seven-day-or-earlier locator and a second owner read. The reviews also add teacher-board acknowledge -> Chat selection, distinguish controlled draft navigation from uncontrolled process replacement, repair zero/partial fanout settlement, forbid terminal-skip backfill and source-link heuristics, and make caregiver replies land on `family_board/recent`. Existing Handoff-id API/deep link/DTO/provider, static bearer, single resolver, current-only broad recipient query, and no candidate ledger/source discriminator/provider lease-retry remain blockers. C-3-4 is `DESIGN COMPLETE / IMPLEMENTATION OPEN`; C-3-5 is next. No contract, source, manifest, schema, migration, route, UI, runtime, typed link, resolver, provider policy, capability, allowlist, database, environment, or traffic changed.

- 2026-07-20: Locked Pilot-0-C3-3-0 through C-3-3e after consolidated product-surface, domain-policy, and cross-repository adoption reviews. `caregiver_family_care_work_v1` is one non-persisted `NurtureFamilyCareItem`-root owner projection across generic Caregiver Chat and teacher board; Attention is only the exact queue index and legacy class-inbox/attention Workflow/internal APIs cannot alias it. Caregiver work requires one exact operational `caregiver` RoleAssignment plus separate Lead designation. Acknowledge is explicit/direct-empty and atomically binds an exact Participant+RoleAssignment Pilot work claim; only that same current claim may perform the explicit/claimed protected reply on the Item's original Grant. Temporary staff/Hold/policy loss is a current fence; terminal claimant loss retains acknowledged/active staffing-review facts with no new-role/Admin/Operator takeover. Reply commits one terminal graph, one logical `user_attention` Handoff, a typed commit-time Guardian audience cohort, and a Host-derived versioned domain-action origin; current delivery planning excludes late join and departed/new roles without creating Notification/provider/open behavior. Caregiver reply redaction is explicit/direct-empty and limited to the same current original staff episode, erases only the reply side, and never reopens/suppresses the source Item. Each body retains independently for 30 days and Caregiver history uses the source-entry 365-day window. The backward review removes the old caregiver AI-draft allowance, resolves lightweight-strong to explicit, changes historical search to owner browse, distinguishes current versus permanent fences, and prevents Attention/legacy presenter dual tracks. Current shared sources/origin, protected runtime/KMS, typed fact graph/claim/audience, Caregiver surfaces, reply Handoff branch, and conformance do not exist, so status is `DESIGN COMPLETE / IMPLEMENTATION OPEN`; C-3-4 is next. No contract, source, manifest, schema, migration, route, UI, runtime, capability, allowlist, database, provider, environment, or traffic changed.

- 2026-07-19: Locked Pilot-0-C3-2-0 through C-3-2f after consolidated product-surface, domain-policy, and cross-repository adoption reviews. `guardian_relationship_authority_v1` is one non-persisted ChildCareProcess-root owner composition over independent relationship, Enrollment/Hold/Transfer, Grant, and StageEpisode roots; the composition never creates a Host or database relationship aggregate. The three Guardian surfaces share current owner semantics, exact episode detail, strong authorization, and static direct/claimed drivers. First/Co-Guardian onboarding uses the separate Host invitation/prospective lane, while accepted self-exit is the new exact `exit_guardian_relationship -> nurture.family_care.exit_guardian_relationship` strong/direct-empty action with last-Guardian denial and complete atomic role/owned-Grant fence. Enrollment confirmation remains exact-invitation-recipient; Hold/transfer/withdrawal and Stage retain equal-Guardian rules; Grant replacement/revoke remains exact Participant+Role owner, with server-offered fixed policy delta only. Stage choice uses owner item/detail reads that finally issue one bound action target rather than a new form protocol. Withdrawal alone adopts the separate default-off `guardian_relationship_attention` producer in C-3-2; C-4 owns Institution producers and C-3-4 owns Notification/open. The consistency review also removed the obsolete conditional Pilot `cancel_route`, clarified that rejoin may restore current family-side retained access but never old original-Grant cross-role authority, and narrowed Enrollment-hold fencing so the fence does not hide family-owned Stage/body-free history. Relationship/Stage history uses owner retention rather than C-3-1's communication 365-day cutoff. Current cross-repo C-3 sources, invitation subsystem, schemas/actions/presenters, relationship attention, recent-auth runtime, and conformance do not exist, so status is `DESIGN COMPLETE / IMPLEMENTATION OPEN`; C-3-3 is next. No contract, source, manifest, schema, migration, route, UI, runtime, capability, allowlist, database, provider, environment, or traffic changed.

- 2026-07-19: Locked Pilot-0-C3-1-0 through C-3-1e after consolidated product-surface, domain-policy, and cross-repository adoption reviews. One Guardian communication entry is a non-persisted Nurture projection rooted in one `NurtureFamilyCareItem`, with orthogonal progress, entry lifecycle, and question/reply visibility rather than a new table or overloaded Item status. Guardian Chat, Family board, and Family workbench share `guardian_family_communication_v1`; current/recent/history are body-free, bounded, owner-reread, and separated by relationship episode. Compose always starts in an empty manual protected editor; ordinary Chat text, AI, legacy `editable_preview`, and `PublicDraft` do not seed it. Submit uses five-minute prepared content plus the pre-bound original claimed Step; source redaction is direct-empty and exact-author/current-family. Equal current Guardians may read family-side source content, but drafts and redaction remain author-local; original Grant permanently fences cross-role work and a new Grant never revives an old entry. Each Message body retains independently for 30 days, while the Guardian entry shell uses a fixed 365-day window from source creation. Adoption is default-off Base -> My-Chat -> Nurture -> isolated joint conformance; canonical manifest generation/parity replaces hand-filtered dual SSOT, the historical `capture_family_input` path cannot alias/fallback, and all persistent capability/Workspace allowlists wait for C-3-5. Current broad input/Grant selection, ThreadParticipant hard gate, synthetic refs, missing typed fact links/Guardian presenter/KMS, and partial `take:100` cascades remain blockers. C-3-1 is `DESIGN COMPLETE / IMPLEMENTATION OPEN`; C-3-2 is next. No contract, manifest, source, generated artifact, schema, migration, KMS key, route, UI, runtime, capability, allowlist, database, environment, provider, or traffic changed.

- 2026-07-19: Locked Pilot-0-C3-0e after owner acceptance, completing the C-3-0 shared-baseline design while leaving implementation open. The atomic `scenario_protected_interaction_v1` depends on trusted ingress, subject presentation, and action execution and covers the complete protected input/store/read/redact/retention/draft/cache/offline path. Five data classes separate process-local unsubmitted draft, five-minute Nurture-encrypted prepared content, committed protected content, at-most-60-second foreground view, and body-free shell/audit. The additive design uses `NurtureProtectedContent`, per-content AES-256-GCM with KMS-wrapped DEK, keyed integrity, typed unique Message linkage, `body=null`, body-free Item/Receipt/Execution/Handoff/Host state, current `read_protected_detail`, and crypto-erasure. A distinct protected composer never uses ordinary Chat, `ContentRevision`, `PublicDraft`, artifacts, browser/PWA cache, or offline recovery. `scenario_protected_ai_draft_v1` is separately gated and off for Pilot-0. Internal Pilot retention is five-minute prepare/15-minute key cleanup, 30-day body, 365-day body-free evidence, zero durable AI content, and 30-day encrypted backup with erasure-ledger replay. `scenario_protected_interaction_source_v1` stays separate from interface/action/historical identities and adoption remains Base -> My-Chat -> Nurture -> joint evidence. Current synthetic refs, plaintext seams, Chat/PublicDraft persistence, and absent contracts/KMS/composer/leakage guards are recorded as blockers. C-3-0 is `DESIGN COMPLETE / IMPLEMENTATION OPEN`; actual adoption is delivered through C-3-1..5, beginning with Guardian family communication. No contract, manifest, source, schema, migration, KMS key, route, UI, renderer, runtime, cache, secret, capability, provider, database, environment, allowlist, or traffic changed.

- 2026-07-19: Locked Pilot-0-C3-0d as one consolidated action-execution/recovery decision after owner acceptance. The atomic `scenario_domain_action_execution_v1` capability depends on trusted ingress and subject presentation and binds each product action to one static `scenario_direct_empty_v1|workflow_claimed_step_v1` driver. All actions use fresh owner prepare/submit contexts; claimed actions persist a content-free non-claimable Step, immutably bind the exact Step in Nurture, then allow only that Step to claim/commit/replay. `submit_family_care_question`, `reply_family_care_item`, transfer proposal, family withdrawal, and Institution service end are claimed; zero recipients or a new `already_satisfied` command cannot change driver or obtain the original seed. Direct-empty preserves the explicit-empty Enrollment/Grant/stage/topology paths. Client command ids are removed from authority; Nurture derives direct/context or claimed/original-Step identities, adds future typed Participant actor evidence without reinterpreting legacy `business_actor_ref`, and atomically consumes context with effect/Execution/snapshots. Results keep business outcome, replay disposition, current presenter, and Host progress distinct. A new `scenario_domain_action_source_v1` remains separate from presentation and historical handoff identities. The B3 `Direct domain action` ambiguity is repaired: the phrase meant non-Run action, while caregiver reply is now explicitly claimed-Step. Invitation, C-0, portability, and Technical Operator remain separate. C-3-0d is planning-complete only; C-3-0e retains protected/draft/cache/offline and actual adoption. No contract, manifest, source, schema, migration, route, runtime, capability, database, secret, provider, environment, allowlist, or traffic changed.

- 2026-07-19: Locked Pilot-0-C3-0c-4 after owner acceptance, completing subject-aware presentation planning. The additive implementation contract uses one atomic `scenario_subject_presentation_v1` Host capability that depends on `trusted_scenario_invocation_v1`; provider, semantic presentation, renderer/freshness/persistence compatibility cannot activate as partial independent paths. Base will add optional versioned provider/presentation registries, strict codecs/validators/bounds, and legacy/vNext/negative fixtures. Any vNext declaration makes missing/duplicate/unknown providers, surfaces, operations, route/reason/action references, Host support, or conflicting legacy operation fatal. Adoption is Base -> My-Chat -> Nurture -> joint conformance. The existing `0bd892...` and `a97a5b...` identities remain historical handoff evidence only; implementation must add `scenario_interface_source_v1`, keep the Nurture Scenario module hash and My-Chat renderer conformance revision separate, and pin exact revisions. A local pnpm `file:` link is not evidence. Nurture keeps one canonical manifest and only a mechanically derived disabled-capability projection. C-3-0c is complete as design/evidence specification, not actual adoption or enablement. C-3-0d action execution/recovery is next; C-3-0e retains protected/draft/offline and complete-IIB adoption. No contract, manifest, source, schema, migration, route, UI, renderer, runtime, cache, secret, capability, provider, database, or traffic changed.

- 2026-07-19: Locked Pilot-0-C3-0c-3 after owner acceptance, with the 2026-07-20 consistency refinement now authoritative: four registered C-3 Nurture product surfaces map internally to My-Chat Chat semantic panel, role-board, or domain-workbench renderer families; neutral Host fixtures cover all three families, and the two Institution keys stay absent until C-4. Renderer/layout primitives stay off the wire. Neutral read-only role-board action offers are fatal mismatches, Teacher board retains complete owner-paginated history, and arbitrary search/sort/compound business filters are explicitly deferred to a later owner query-control contract. Presentation data is classified as `owner_canonical`, `ephemeral_presentation`, `durable_host_shell`, or `forbidden_host_copy`; semantic copy/blocks/offers/refs/AI projection never become durable Chat/dashboard content, while a content-free Chat rehydration marker may persist. Foreground freshness is capped at 60 seconds with immediate known invalidation, clearing before refresh, and no cached/stale/legacy/offline fallback or optimistic Nurture state. Locale, semantic reading order, focus, keyboard/screen-reader, scaling, reduced-motion, and safe transition announcements are renderer conformance. Current broad My-Chat interaction/dashboard paths and absent role boards/workbenches remain implementation gaps. C-3-0c-4 exact conformance/adoption is now locked by the entry above; C-3-0d/e retain action execution and protected/draft/offline behavior. No contract, manifest, source, schema, migration, route, UI, renderer, runtime, cache, secret, capability, provider, database, or traffic changed.

- 2026-07-19: Locked Pilot-0-C3-0c-2 after owner acceptance. The read-only `present_subject_context` contract now returns only `ready|empty|context_changed|unavailable`. Nurture resolves BCP-47 plain text and safe reasons; My-Chat does not host Nurture dictionaries or translate reason/action codes. Semantic output is limited to flat `summary|notice|fact_group|metric_group|item_collection|timeline` blocks, separate non-durable navigation offers, and entitled domain-action offers. Available actions carry opaque prepare-only target/version evidence plus explicit confirmation, priority, tone, and narration policy; unavailable actions carry safe reason only when current disclosure is permitted and are otherwise omitted. Collection/timeline item refs open only current owner-read detail and cannot become action targets. AI projection strips every ref/version/code/cursor/action target and includes only `narration=allowed` safe text. Bounds are 64 KiB, 20 blocks/entries, 8 actions/navigation offers, exact text caps, and five-minute cursor/item/continuation/action-target refs. Existing Base Run action availability, broad My-Chat mobile interaction envelopes, and Nurture id-shaped pseudo-opaque refs remain implementation gaps. C-3-0c-3 was the next checkpoint and is now locked by the entry above; C-3-0d/e retain action execution and protected/draft/offline handling. No contract, manifest, source, schema, migration, route, UI, renderer, runtime, cache, secret, capability, provider, database, or traffic changed.

- 2026-07-19: Locked Pilot-0-C3-0c-1 after owner acceptance. Subject discovery and current revalidation are separate `list_subject_contexts` and `resolve_subject_context` operations. The provider uses a new bounded opaque `ScenarioSubjectContextRefV1` rather than `DomainContextRef`; Nurture binds Workspace, scenario, provider, verified principal, owner locator, scope, version, issue time, and expiry internally. A collection is one `subject_collection` context and never expands member identities or counts. Closed `resolved|needs_selection|context_changed|unavailable` results expose only an opaque ref, generic scope/route classes, owner-safe label slots, opaque context version, and bounded issue/expiry metadata. Context refs expire within 30 minutes, cursors within 5 minutes, page size defaults to 10 and cannot exceed 20. The ref is a locator only; every action prepare/submit and surface transition re-resolves current authority. C-3-0c-2 retains exact safe-label encoding and blocks/actions. No contract, manifest, source, schema, migration, route, UI, runtime, cache, secret, capability, provider, database, or traffic changed.

- 2026-07-19: Locked Pilot-0-C3-0c-0 after owner acceptance. The presentation pipeline is now verified My-Chat invocation -> current Nurture subject/owner resolution -> display-safe semantic presentation -> My-Chat generic surface adapter/renderer. Base owns reusable shapes/validation only; Nurture owns relationship and business semantics, safe disclosure, and action availability; My-Chat owns shell, generic Chat/card/list/detail/workbench rendering, navigation, accessibility, and device adaptation without business interpretation. Chat remains AI conversation plus generic structured UI, and narration may use only the current display-safe owner output. Presentation is never authorization or replay evidence; every seam rereads current Host/Nurture state, and unclassified presentation/domain/protected fields cannot enter Host persistence. Exact provider wire, block/action vocabulary, renderer/persistence classes, and conformance remain C-3-0c-1 through C-3-0c-4; C-3-0d/e retain action-driver and protected/offline policy. No contract, source, schema, migration, route, UI, renderer, runtime, cache, secret, capability, provider, database, or traffic changed.

- 2026-07-19: Completed the post-lock C-3-0b consistency repair and advanced the checkpoint to C-3-0c discussion. The detailed C-3-0b heading now matches the locked/complete status recorded by overview, plan, roadmap, and verification. The review also supersedes the shorthand that made Host establishment sound like ownership of Nurture command semantics: clients cannot author `command_request_id`; My-Chat establishes only current request/driver inputs and any bound authentication-assurance evidence; Nurture validates canonical business-effect identity/hash/replay under the registered operation/driver contract. A Host assertion proves authentication assurance only, while the Nurture strong-authorization decision still requires current owner facts, policy, owner-issued submit context, exact action, and explicit confirmation. Exact direct-empty versus claimed-Step command derivation remains an explicit C-3-0d decision and cannot be inferred from C-3-0b. No C-3-0c presentation decision was pre-locked, and no contract, source, schema, migration, route, UI, runtime, audit sink, secret, capability, provider, database, or traffic changed.

- 2026-07-19: Locked Pilot-0-C3-0b-4 after owner acceptance, completing authenticated-ingress planning. The contract separates strict discriminated client echo, Host-established identity/route/request/driver/strong-auth evidence, and Nurture-owner Participant/role/Subject/Grant/policy facts. Signed echo remains untrusted; free-form payload/display/details and client-authored command identity are forbidden. Host ingress HTTP states remain separate from current Nurture owner safe presentations; verifier defects and existence-sensitive failures collapse safely. Owner-split audit stores no protected body or credential and uses Pilot defaults of nonce hash <=5m, traces 7d, logs 14d, aggregate/ordinary ingress audit 90d, and provisioning/operator/owner-recovery audit 365d. Business content retention remains the later P1/C-3-0e gate. Base -> My-Chat -> Nurture adoption is additive, exact-revision/hash and registry pinned, default-off, negative/leakage/replay gated, and no-fallback. Review resolves the overlap with C-3-0e: C-3-0b-4 adoption covers authenticated ingress only; complete IIB adoption stays later. Current broad envelope/surface/error objects and missing audit/conformance remain implementation gaps. C-3-0b is complete and C-3-0c is next. No contract, source, schema, migration, route, UI, runtime, audit sink, secret, capability, provider, database, or traffic changed.

- 2026-07-19: Locked Pilot-0-C3-0b-3 as one consolidated ingress-variant decision after owner acceptance. The versioned ingress context now discriminates registered Nurture product surfaces (`nurture_chat`, family/teacher/institution boards, and family/institution workbenches), Host transitions (`notification_open|invitation_continuation`), and the sole workflow-runtime surface (`workflow_worker`). The ordinary caller matrix stays closed: API/interactive handles product and transition ingress, while worker/durable handles only runtime ingress and explicitly durable-enabled operations. Notification is a two-stage current owner flow rather than provider/deep-link authority. Durable replay reconstructs the current Run actor, validates the separately claimed original Step, and never impersonates a UI surface. Post-accept onboarding reuses the signed human-principal transport but is the sole exact-invitation-bound application exception that may create/reuse a missing Participant atomically without exposing ordinary Subject data before commit. C-0 bootstrap and Technical Operator owner reevaluation require separate versioned callers/audiences/verifiers/gates and never construct an ordinary human principal or Nurture role surface. Cross-path replay/substitution is denied; surface remains eligibility/audit context rather than command identity. Review also identified and resolved the planning-level zero-Participant scope ambiguity: ordinary paths fail closed, while the explicitly registered prospective transaction owns the only exception. Existing broad `client_surface`, optional Host identity, legacy event kinds, worker/workbench labels, owner-read token, mixed actor persistence, and missing contracts/routes remain implementation gaps. C-3-0b-4 is next. No contract, source, schema, migration, route, runtime, secret, capability, provider, database, or traffic changed.

- 2026-07-19: Locked Pilot-0-C3-0b-2 after owner acceptance and a consolidated consistency review. Private subject-aware invocation now requires an allowlisted My-Chat workload credential plus a separately ES256-signed exact-body `ScenarioPrivateInvocationV1`. The versioned `ScenarioHumanPrincipalV1` contains only exact My-Chat User, human Actor, Workspace, and interactive-session/durable-Run-actor origin; Workspace plus User binds one Nurture Participant, while Host Actor remains provenance. The signed envelope binds exact caller, environment-specific audience, contract hash, endpoint/method/scenario/surface, request/correlation evidence, payload, and a 60-second single-use nonce. A fresh envelope/new nonce recovers response loss while the inner `command_request_id` and original Step provenance preserve CommandExecution replay; replay storage keeps only nonce hash/expiry. ES256 key rotation uses prepublish/current/previous overlap, emergency revocation, fixed key sources, and environment isolation. Strict validation terminates before Participant lookup and produces only a normalized verified context. `trusted_scenario_invocation_v1` is additive and cannot fall back to legacy `actor_id`, default/inferred workspace, broad `client_surface`, owner-read token, legacy handler, or a second authenticator. Review found that current `business_actor_ref` is already polymorphic: legacy family-core writes My-Chat/system refs while institution commands require Participant id. C-3-0b-2 forbids Host Actor as C3 business authority without silently reinterpreting historical rows; C-3-0d must choose an additive typed/versioned actor field or explicit migration. Current Base/My-Chat/Nurture types, static owner-read token, optional identity metadata, mixed business-actor persistence, missing nonce/key/verifier path, and missing capability remain implementation gaps. C-3-0b-3 path variants are next; C-3-0b-4 retains client echo, public denial, audit-retention, and adoption evidence. No contract, source, schema, route, runtime, secret, capability, provider, database, or traffic changed.

- 2026-07-19: Locked Pilot-0-C3-0b-1 after owner acceptance. My-Chat now has three planning-level Host context modes: `platform_general` for generic Q&A with no business workspace/Nurture access, `workspace_business` as the only ordinary subject-aware mode, and `invitation_acceptance` as a pre-membership exact-recipient Host transition. A personal workspace may remain an internal Chat storage partition but cannot silently become business context. General-to-Nurture transition is explicit even with one eligible workspace, creates/enters a workspace-scoped conversation, and carries only the current confirmed intent by default. Existing resources bind workspace server-side; explicit business routes may use shell selection or sole membership; multi-workspace ambiguity, client mismatch, session/membership loss, and personal fallback fail closed. Invitation acceptance commits Host membership independently, then offers a separately gated/idempotent Nurture continuation with no cross-DB transaction or automatic business effect. Stable public states distinguish sign-in, account readiness, workspace selection/access, scenario availability, and safe owner denial. Current personal defaulting and missing adult Workspace Invitation model/route are readiness gaps. C-3-0b-2 private service/principal envelope is next; no contract, schema, route, source, runtime, secret, database, capability, provider, or traffic changed.

- 2026-07-19: Locked Pilot-0-C3-0b-0 after owner acceptance. My-Chat is the sole public business ingress; a trusted Nurture invocation independently proves the My-Chat workload and carries one Host-established adult principal containing only account, human actor, exact validated workspace, scenario/surface provenance, request evidence, and principal origin. Service identity, workspace membership, surface, and Technical Operator never become Nurture role or Subject authority. Nurture binds Participant from account and rereads current RoleAssignment/Subject/policy for every interaction; Host actor is provenance only. Client workspace selection and action/token/version echoes remain untrusted inputs. Invitation acceptance stays a separate Host identity/membership transition, worker replay later derives the adult from durable Run actor/current membership, and secrets/tokens are never persisted or logged. Current optional `actor_id` and workspace-optional Nurture envelope are legacy scaffold only; activated ingress requires an additive versioned path. C-3-0b-1 public ingress/session/workspace is next; exact public routes, private DTO names, contract package, manifest, source, schema, runtime, secret, capability, provider, database, and traffic remain unchanged.

- 2026-07-18: Locked Pilot-0-C3-0a after owner acceptance. My-Chat's user-facing business scenario catalog now converges on education/nurture, while reusable Chat/Workflow/Notification/Forum/Knowledge infrastructure remains subject-neutral. Product language stays child-centered, but shared contracts use generic `subject`; adult My-Chat accounts remain actors rather than child identity. Every activated business scenario must expose a scenario-owner Account–Subject reachability provider starting from the authenticated account and resolving `unresolved|single_subject|subject_collection`. Nurture derives Guardian through Family to ChildCareProcess, Caregiver through CareGroup/Enrollment to a collection, and Institution Admin through Institution topology to a collection; invitation-only context remains prospective/minimal and may issue only the exact onboarding action, while Technical Operator has no subject relationship. My-Chat may aggregate only workspace/scenario-bound opaque entries for discovery/navigation, never a second Child/relationship SSOT or authorization cache. No established relationship means no ordinary subject presentation/action context; every render/action owner-rereads. Subject context creates no bulk authority, account-for-child, global identity, or cross-workspace/cross-scenario link. C-3-0b authenticated ingress/trusted principal is next; exact wire fields, provider implementation, index projection, manifest, contract package, source, schema, migration, runtime, environment, capability, provider, database, and traffic remain unchanged.
