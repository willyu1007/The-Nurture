...
- Fix / workaround: C-3-0b-4 separates strict `ScenarioClientEchoV1`, Host-only context, and Nurture-owner-only facts; derives command identity at the Host; exposes only layered safe outcomes plus opaque support refs; splits body-free audit by owner; and fixes purpose-specific access and retention. Protected family content remains under its own later business-data policy.
