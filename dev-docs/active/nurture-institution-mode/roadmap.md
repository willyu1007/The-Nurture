...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 的 C-2f-2 已锁定。Same-Institution transfer 使用七天 `NurtureEnrollmentTransferIntent`，Institution workbench propose/cancel，any current Guardian confirm/decline；不复用 initial invitation、initiate/close key 或 direct transfer。Proposal/confirmation 要求 zero active hold 并绑定 source version；target Group active/ready，source Group 可 active/paused/archived transfer-out。Additive exact `rosterEntryId`、unique new-to-old `supersedesEnrollmentId` 和 terminal reason/Execution 支持 target-on-confirm roster 与 no-mirror lineage。One Serializable database-time transaction end old Enrollment、完成 C-2e Grant/work closure、archive old Thread/roster、create target roster/new active Enrollment、consume intent；overflow/fault 整体回滚。No old Grant/Thread/content/work/hold/context/Handoff/notification/policy authority transfers；target 只得 safe roster，fresh Grant 才建 new Thread。C-2f-3 permanent exit/re-entry、C-2f-4/5、operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
