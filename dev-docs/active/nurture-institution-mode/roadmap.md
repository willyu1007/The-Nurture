...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2f-4-2 locks current-Guardian longitudinal aggregation, repository-scoped Institution isolation, no cross-Institution existence/id/stage leakage, no stage dataClass, independent per-Institution lifecycle/concurrency, exact dependency rereads, and safe stale-episode degradation; C-2f-4-3 future protocol boundary next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 的 C-2f-4-2 已锁定。Current exact-Family Guardian 可按 ChildCareProcess 聚合 family-owned StageEpisode 与每个 Institution 的 safe Enrollment episode summary，但 protected objects 继续绑定原 Enrollment/Grant/role/redaction/retention。Institution Admin/Caregiver query 必须从 current exact Institution/CareGroup role 出发并在 repository 层过滤；不能先查全 process 再应用层过滤，也不暴露 other-Institution existence/count/raw process id/route/error/body/stage/history。Enrollment/roster 不授予 stage visibility，Pilot 不新增 stage dataClass。Current Enrollment uniqueness 仍按 `(workspace, process, Institution)`；A 的 lifecycle/policy/Grant 不影响 B，不相关 stage projection 变化不能成为 Enrollment conflict。Family aggregate segment owner-read stale/failure 时只显示 safe unavailable，不回填 cache。C-2f-4-3 future cross-workspace protocol boundary、C-2f-5 result/delivery、operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
