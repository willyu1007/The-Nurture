...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B and Pilot-0-C2 complete; C-3-0b authenticated ingress and C-3-0c presentation planning complete; C-3-0c-4 locks atomic capability, additive contracts, named adoption evidence, ordered three-repo adoption, legacy/no-fallback behavior, and conformance; C-3-0d is next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B、Pilot-0-C2、C-3-0b authenticated-ingress planning 和 C-3-0c presentation planning 已完成。C-3-0c-4 以 `scenario_subject_presentation_v1` 原子能力、`trusted_scenario_invocation_v1` 依赖、additive manifest、独立 `scenario_interface_source_v1`、Scenario module/Host renderer 身份、Base -> My-Chat -> Nurture 顺序采用、机械 pre-activation projection、legacy 兼容/no fallback 和四层 conformance 关闭 presentation 设计门禁。当前 `0bd892...` 仍只证明 handoff 基线，不能充当 presentation adoption。以上仍是 planning-only，未满足实际三仓采用或 capability/allowlist 启用。C-3-0d/e、Guardian/Caregiver IIB、C-4 Institution IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
