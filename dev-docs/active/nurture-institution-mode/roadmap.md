...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C locked through C-2b-3 Guardian rights/history; C-2b-4 relationship exit/revoke next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 已锁定到 C-2b-3。两位当前 Guardian 的基础家庭权限平等，并可按当前 Family/role/Enrollment/original-Grant/redaction/retention/policy owner-reread 合格的既有事实；但作者权、Grant 管理权和跨角色正文授权保持独立。新 Guardian 不共享未提交草稿或 InteractionContext，不复制历史、不补发旧通知，也不能恢复撤销/红删/过期内容或获得任意档案编辑权。C-2b-4 relationship exit/revoke、C-2c-f、后续 operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
