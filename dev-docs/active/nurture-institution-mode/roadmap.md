...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2e-1 locks any-current-Guardian review, first-committer ownership, mandatory strong-confirmation disclosure, and exact submit context; C-2e-2 atomic Grant/Thread semantics next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 的 C-2e-1 已锁定。任一当前 exact-family Guardian 都可首次 review/confirm，第一位成功提交者成为 sole Grant owner；其他当前 Guardian 可使用但不能 replace/revoke。Chat、family board、family workbench 使用同一 `confirm_child_link_grant`，必须展示 child/Institution/CareGroup、双向 question workflow、期限、owner、撤销/保留和明确排除项，并通过五分钟 exact actor/surface `submit_action` 完成产品级强确认；自然语言、导航、raw id、客户端 Grant 字段或跨 surface/device context 均不授权。Existing exact active Grant 只返回 current state/`already_satisfied`，不转移 owner。C-2e-2 atomic Grant/Thread identity/time/expiry/concurrency、后续 recovery/revoke、C-2f、operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
