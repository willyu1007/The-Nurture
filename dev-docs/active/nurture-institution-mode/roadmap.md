...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2e-3 locks immutable receipt/current presentation, safe result/actor semantics, exact response-loss replay, no result token, and explicit-empty Grant confirmation; C-2e-4 lifecycle/cascade next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 的 C-2e-3 已锁定。Grant Execution 固定 immutable `executed|replayed` 与 `applied|already_satisfied` 语义和 exact Grant/Thread refs；`family_care_grant_current` 每次 owner-reread，区分 `activated|already_active|processed_but_unavailable` 与 `owner|family_user|none`，不会把第二 Guardian 描述成 confirmer/owner。Response loss 在 consumed/expired context 前 exact replay，不需要二次确认；丢失 command identity 只能查询 current state，不能创建 probe command/result token。Grant confirm 永远 explicit `[]` + null driver，不创建 Step/Handoff/Outbox/notification/deep link/teacher visibility/protected object。C-2e-4 replacement/revoke/owner loss/cascade、C-2f、operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
