...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C locked through C-2b-1 first-Guardian establishment; C-2b-2 Co-Guardian Invitation next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 已锁定到 C-2b-1。机构本地 RosterEntry 不证明长期孩子或 Guardian 身份；首位 Guardian 必须是准确受邀并认证的成人，经强确认后由单一 Nurture 事务原子建立 Participant/Child/Process/Family/首个 Guardian 关系。既有档案只允许当前同 workspace Guardian 选择；没有 `primary_guardian`，家庭关系标签不改变权限，Pilot 也不宣称法律监护核验。C-2b-2 Co-Guardian Invitation、C-2c-f、后续 operational IIB、real cohort sizing、isolated topology、workspace allowlist、成功/停止和回滚仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
