...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2f-3c locks same-Institution fresh re-entry with existing actions, new roster/invitation/Enrollment identities, generalized predecessor lineage, atomic current-owner confirmation, episode-separated history, and no old authority revival; C-2f-4 portability next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 的 C-2f-3c 已锁定。同一 Institution 的 fresh re-entry 由 Institution workbench 从 exact historical Enrollment 发起，复用 `initiate_enrollment`；exact invited current Guardian 在三种 Guardian surface 复用 `confirm_family_enrollment`。Child/ChildCareProcess/Family/current Guardian 延续，但 RosterEntry、Enrollment Invitation、context、Execution、Enrollment 及后续 Grant/Thread 都使用新 identity。新 Enrollment 以 `predecessorEnrollmentId + continuityKind` 记录唯一单向 lineage；该设计同时覆盖 transfer 并取代尚未实施的 `supersedesEnrollmentId`，禁止双 lineage SSOT。Predecessor 必须是同 workspace/process/Institution、已完成 C-2f-3b closure、无 successor 且没有 current same-Institution Enrollment 的 terminal leaf。Old/new episodes 分离展示，old cross-role body、Grant、Thread、work、notification authority 均不恢复。C-2f-4 portability、C-2f-5 result/delivery、operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
