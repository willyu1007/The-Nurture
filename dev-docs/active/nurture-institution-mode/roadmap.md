...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2f-3b locks one Enrollment-rooted atomic terminal closure, complete bounded Hold/Grant/Intent/Thread/roster/work convergence, Enrollment-first topology ordering, deterministic replay/races, and rollback on any survivor; C-2f-3c fresh re-entry next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 的 C-2f-3b 已锁定。Guardian withdrawal、Institution service end 与 transfer 共享 Enrollment-rooted Serializable closure：统一先锁 Enrollment，再处理 Hold、pending TransferIntent、roster、按主键排序的 Grant、Thread 与 C-2e dependents；该顺序取代早期 C-2f-2 Intent-first 表述。一个数据库时间内写入 immutable terminal audit，system-close Holds，按当前有效或已过期分类收敛 Grants，invalidate pending intents，关闭 Thread/roster/work，并执行 zero-survivor assertions。任何 fault、phantom、overflow、integrity defect 或 survivor 都整体 rollback 并触发 Pilot stop/manual reconciliation；exact replay 稳定，fresh same-cause duplicate 不得改写 actor/time/reason。C-2f-3c fresh re-entry、C-2f-4/5、operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
