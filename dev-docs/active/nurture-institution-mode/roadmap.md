...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B and Pilot-0-C2 complete; C-3-0b authenticated ingress complete; C-3-0c-0 now locks the Nurture semantic-presentation/My-Chat generic-renderer ownership pipeline and C-3-0c-1 subject-provider wire contract is next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B、Pilot-0-C2 和 C-3-0b authenticated-ingress planning 已完成，C-3-0c-0 presentation pipeline/ownership 已锁定。客户端不能提供 `command_request_id`；My-Chat 只建立 request/driver 与 authentication-assurance evidence，Nurture 按 operation/driver contract 校验 canonical effect identity/hash/replay 与业务授权。presentation pipeline 固定为 verified My-Chat invocation -> Nurture current subject/owner resolution -> display-safe semantic presentation -> My-Chat generic Chat/card/list/detail/workbench renderer。Chat 是 AI 对话加通用结构化 UI，AI 和 renderer 均不能解释或发明 Nurture role、Subject、safe reason、lifecycle 或 action availability；presentation 不是授权，Host 也不能持久化 raw Subject/relationship authority/protected body/owner token/inferred business state。以上仍是 planning-only。C-3-0c-1 subject-provider wire、C-3-0c-2 blocks/actions、C-3-0c-3 renderer/persistence、C-3-0c-4 conformance/adoption、C-3-0d/e、Guardian/Caregiver IIB、C-4 Institution IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
