...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2e complete after C-2e-4d locks typed permanent cascades, reversible fences, root-first loop-to-closure, bounded audit/overflow, redaction branches, and stale Host delivery; C-2f next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 的 C-2e 已在 C-2e-4d 后完成。Permanent Grant/source invalidation 使用 typed `NurtureInteractionContextDependency` 和一个 root-first Serializable cascade kernel；temporary Host/role/policy/topology unavailability 只做 current fence。Kernel preflight hard cap、同事务 keyset loop、禁止 `SKIP LOCKED`/中间 commit、最终 `NOT EXISTS`，并用 bounded `NurtureLifecycleCascadeAudit` 保存 counts/hash/closure evidence；overflow 在 root mutation 前整体回滚。Grant invalidation 保留受保护 Message 与 audit，清除 cross-role/secondary projections；source/reply redaction 保持不同 cascade。Old replay seed 可保持 immutable refs-only，但 consumer/provider/open 每次 owner-reread。所有 invalidation paths 永远 explicit `[]` + null driver。C-2f、operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
