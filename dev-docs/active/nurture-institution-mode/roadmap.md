...
| D8 | IIA implementation entry | **IN PROGRESS:** N1-A through N1-E are static/unit complete: additive schema, `NurtureCommandExecution`, `NurtureInteractionContext`, fail-closed resolver/policy, family-care command transitions, inbox and attention owner reads all remain explicit `[]`; production migration/DB-backed surface acceptance is N1-F | Apply migration only to an approved target, pass DB-backed journeys, then add direct surface handler/presenter conformance without enabling non-empty activation |
