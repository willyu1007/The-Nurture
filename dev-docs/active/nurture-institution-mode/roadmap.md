...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A actual-capability audit complete; cohort/IIB/topology/success-stop/rollback recommendations await acceptance | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5 已完成，Pilot-0-A 跨仓实际能力审计已完成，结论是 readiness 可继续、external traffic NO-GO。`09-pilot-readiness.md` 中的 cohort/data class、IIB/onboarding、isolated topology、workspace allowlist、成功/停止和回滚建议等待接受或修订；Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
