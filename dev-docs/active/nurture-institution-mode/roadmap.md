...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2e-0 locks owner-reread authority and optional non-authorizing ThreadParticipant projection; C-2e-1 Grant review next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 的 C-2e-0 已锁定。当前 Host/Nurture identity、Participant/RoleAssignment、exact Enrollment/CareGroup、exact current/original Grant、Thread/source lifecycle、policy 和 redaction 组成唯一 owner-reread 权限路径；ThreadParticipant 仅是可选 routing/read/subscription/UI projection，缺失不拒绝当前合法权限，陈旧或伪造行不授予或保留权限。Exact Thread 仍是必需业务聚合，现有 `thread_membership_active` 硬门禁必须在实施时移除。C-2e-1 Grant review/confirming-Guardian authority、后续 Grant atomic/lifecycle/recovery/revoke、C-2f、operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
