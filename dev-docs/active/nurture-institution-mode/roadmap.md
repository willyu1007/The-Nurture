...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C in progress with C-0/C-1 and C-2a no-existing-profile/RosterEntry boundary locked; C-2b Family/Co-Guardian Invitation next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成。Pilot-0-C 的 C-0、C-1 与 C-2a 已锁定：My-Chat-only public ingress、Nurture private authority、一次性 Institution bootstrap、唯一 `NurtureCareGroup`、派生 readiness、Staff Invitation 与独立 Caregiver/Lead 角色，以及无既有产品档案家庭的最小机构本地 `NurtureInstitutionRosterEntry` 入口。RosterEntry 不是长期孩子档案、Enrollment 或 Grant；只有认证 Guardian 才能创建/选择 family-authorized child process，之后分别确认 Enrollment 与 Grant。C-2b Family/Co-Guardian Invitation、C-2c-f、后续 operational IIB、real cohort sizing、isolated topology、workspace allowlist、成功/停止和回滚仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
