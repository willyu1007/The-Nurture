...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2f-0 locks current/terminal lifecycle, family/institution actor classes, independently attributed reversible pause, permanent old-Enrollment closure, new-identity transfer, Institution isolation, and no automatic cross-workspace migration; C-2f-1 next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 的 C-2f-0 已锁定。Enrollment `active|paused` 属于 current，`ended|withdrawn` 属于 terminal 且不得 reactivation，legacy `pending` 只作 conflict-bearing，`deleted` 不是 Pilot action。Guardian 只拥有 family-side restriction/withdrawal actor class；exact-scope Institution Admin 只拥有 institution-side restriction/end/transfer-proposal actor class；Caregiver、Lead、Operator、service identity、AI、raw id 和 ambient Host authority 均不能改变 topology。Pause 是 independently attributable reversible fence，任何一方不能 release 另一方 restriction，也不能触发 permanent cascade。Withdrawal/end/completed transfer 必须 closure old Enrollment/Grant work；transfer 创建 new Enrollment identity 而不是 edit `careGroupId`，且不能 carry old Grant/Thread/content authority。Cross-Institution Enrollment 独立，cross-workspace 自动 matching/migration 被禁止。C-2f-1 pause/resume、C-2f-2..5、operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
