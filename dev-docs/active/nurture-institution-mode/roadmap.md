...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2c-1/2/3 issue, child branch, and lifecycle locked; C-2c-4 result/handoff next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 已锁定到 C-2c-3。Enrollment Invitation 持久化 `pending|consumed|cancelled|superseded`，到 `issuedAt + 168h` 即实时过期；管理员取消与收件人拒绝使用不同 reason。任何 correction/reissue 都生成新 intent/Host ref/expiry/hash 和 supersede lineage，provider retry 仍是原 intent。readiness loss 只派生 blocked；恢复且未过期可继续。终止状态使 InteractionContext 失效但保留独立家庭档案，C-2d Enrollment 事务是唯一 consumed 写入方，所有并发 first-commit-wins。C-2c-4、C-2d-f、后续 operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
