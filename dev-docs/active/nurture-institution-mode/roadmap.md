...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2c-1 Enrollment Invitation issue/binding locked; C-2c-2 acceptance branching next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 已锁定到 C-2c-1。只有当前 Institution Admin 可在完整 C-1 readiness 下，为准确 CareGroup、未链接 RosterEntry 和 opaque Host recipient binding 创建一个带 expiry/version/hash 的 Nurture Enrollment Invitation intent；每个 RosterEntry 同时只有一个有效 pending intent。My-Chat 拥有联系方式、投递、认证和 Host acceptance，Nurture intent 才是业务继续权威。发出邀请不创建 Participant/Guardian/Child/Family/Enrollment/Grant/thread 或老师可见事实，三个 RosterEntry 分别使用三份邀请，Co-Guardian invitation 完全分离。C-2c-2 acceptance branching、C-2c-3/4、C-2d-f、后续 operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
