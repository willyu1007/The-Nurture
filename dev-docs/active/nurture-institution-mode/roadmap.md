...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2f-4-3 keeps cross-workspace `NO-GO` and locks a future same-adult, dual-owner-read, fresh-local-identity portability boundary with a minimum profile allowlist, no stage/authority/content carryover or match/merge, bounded target binding, exact replay, pre-consume revoke, and no post-import source recall; C-2f-5 next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 的 C-2f-4-3 已锁定。Pilot 仍禁止 cross-workspace lookup/match/reuse/import；未来首版仅允许同一个 My-Chat 成人账号在源、目标 scope 分别通过 Guardian/owner reread 和强确认，把显式选择的 `displayName` 与可选 `birthDate` 复制为新的 target-local Child/Process/Family/first-Guardian identities。StageEpisode/history、Institution/Enrollment/Grant/Thread/content/media/health/contact/raw ids/technical state 均不携带；已有目标档案不匹配、不合并。Future Intent 固定七天并绑定 opaque Host recipient 与目标 workspace，最终 context 五分钟；pending revoke/expiry/redaction 拒绝，exact replay 只恢复一个目标结果，commit 后源端不能远程删除目标事实。My-Chat 仅负责账号认证和 refs-only 交付，Nurture 拥有未来业务 snapshot/consumption/idempotency。C-2f-5 result/delivery、operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
