...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2b Guardian lifecycle complete; C-2c Institution Enrollment Invitation next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 的 C-2b Guardian lifecycle 已完成。接受后的关系只允许在仍有另一位当前 Guardian 时自助退出；同级 Guardian、机构、老师和技术操作员不能强制移除。退出事务原子终止自身角色、本人发出的待接受邀请、本人拥有的 active Grants 及既有依赖，并立即阻断 owner reread；Participant、作者关系、业务事实和审计保留。Host loss 不改 Nurture 关系，重新加入需要新邀请/新角色，旧 Grant 不复活。C-2c Institution Enrollment Invitation、C-2d-f、后续 operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
