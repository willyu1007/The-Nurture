...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C in progress with C-0 ingress/bootstrap locked and C-1 adult identity/role onboarding next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A 与 Pilot-0-B 已完成。Pilot-0-C 已开始，C-0 锁定 My-Chat authenticated public ingress、Nurture private owner/action boundary，以及一次性 allowlisted Institution bootstrap；该 bootstrap 既不授予普通 workspace admin 或 Technical Operator 业务权限，也不允许 dev-host、客户端角色声明或直接数据库写入。C-1 成人邀请/participant/role 生命周期、后续 Guardian/Caregiver/Institution IIB、real cohort sizing、isolated topology、workspace allowlist、成功/停止和回滚仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
