...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2 is complete and C-3-0b-2 now locks independent workload/signed-human proofs, exact User/Actor/Workspace/origin, request/contract binding, bounded nonce versus command replay, key rotation, strict codec, and no-fallback vNext activation; C-3-0b-3 next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 和 Pilot-0-C2 已完成。C-3-0b-2 要求每个 subject-aware private call 同时具备独立 My-Chat workload credential 与 ES256-signed `ScenarioPrivateInvocationV1`。`ScenarioHumanPrincipalV1` 只含 exact User、human Actor、Workspace 和 interactive/durable origin；Nurture 仅以 Workspace+User 绑定 Participant，Host Actor 保持 provenance。60 秒 envelope、single-use nonce、raw-body/route/contract binding 与内层稳定 `command_request_id` 将 transport replay、business replay 和 original-Step provenance 分开。Caller/audience/key rotation、strict verifier、credential/log boundary 和 `trusted_scenario_invocation_v1` no-fallback gate 已设计锁定但尚未实现，现有 owner-read token、optional `actor_id` 和 broad `client_surface` 不可激活。C-3-0b-3 ingress variants、后续 Guardian/Caregiver IIB、C-4 Institution IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
