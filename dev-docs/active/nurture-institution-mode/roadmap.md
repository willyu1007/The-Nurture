...
> 宏观规划：里程碑、范围、依赖、回滚。Pilot-0-C 当前状态与术语先看 `10-pilot0-c-current-decision-index.md`，Pilot-0-D 拓扑/运营决策看 `11-pilot0-d-topology-operations-contract.md`；细节看 `01-plan.md`，边界/接口看 `02-architecture.md`，IB schema 草案看 `06-ib-nurture-schema-spec.md`，详细决策账本看 `09-pilot-readiness.md`。
...
| Wave 4 P1/P2 repair | My-Chat/T-028 + The-Nurture/T-002 | Exact source and native CI green | Host `30792cd` / `30375174861` and Nurture `1db91b3` / `30375174703` pass with zero annotations. Formal adoption review remains; P3 stays held. |
...
| 所有权边界 | My-Chat owns account + protected platform Child/Family identity/bindings + shell；Nurture owns body-free binding anchors, exact local associations, workspace-local care dossiers/roles/Enrollment/Grant and care facts。Binding/anchor alone never authorizes business access. |
...
| Pilot-0-D topology/operations | Dedicated Hangzhou Pilot VPC uses separate My-Chat/Nurture ECS and RDS owner boundaries, My-Chat-only Redis/public ingress, immutable OCI candidate, ACR only at Pilot-1, Alibaba Secrets Manager/KMS, dual technical gates, disable-only Technical Operator, isolated `15m/4h` restore objectives, and a fresh-row uninterrupted 120-hour internal observation contract. D is design-locked only and grants no implementation/E/Pilot-1 authority. |
...
| 首批 capability | `class_family_inbox`、`teacher_attention_board`、`caregiver_daily_care`、`child_media_attribution`；当前内部 Pilot 仅验证 responsive Web Chat/board/workbench 与 in-app Notification/Web deep link，native mobile 是 post-Pilot surface direction，不是业务 capability。 |
| 机构价值 | 理念传达、资产沉淀与再组织、运营质量管线；首轮以 responsive Web 的老师执行减负为入口，native mobile 留到 post-Pilot。 |
...
| D2 | My-Chat/Nurture 边界 | **LOCKED / C-4-5 RECONCILED:** My-Chat 拥有 account、protected platform Child/Family identity/binding 和 shell/runtime；Nurture 拥有 typed anchors、workspace-local associations 与全部托育业务事实；平台 binding 与本地 subject/authority 严格分层 | Qualified `platform_child_family_identity_source_v1` before C40 |
| D3 | 角色与 surface | **LOCKED / B3 RECONCILED:** Guardian 使用通用 AI Chat、family board、family Web workbench；Caregiver 使用通用 AI Chat 与 teacher board；Institution Admin 只使用 institution board/workbench，不进入 Nurture Chat；Technical Operator 只使用 My-Chat Technical Admin。Surface 不选择或切换可信角色，Nurture 每次解析当前 Participant/role/scope/target/policy。 | 仅在 post-Pilot 新产品范围评审中增加 surface/persona。 |
...
| D6 | 首批 capability | **LOCKED:** 四个 capability key 分开；当前内部 Pilot 只激活 responsive Web surface，native mobile 为 post-Pilot；实现先做 `class_family_inbox` + `teacher_attention_board` | Done |
...
| IIB | 操作台 + 同意 UX（本仓） | 当前 Pilot：机构/老师 responsive Web 工作台 + 家长同意/撤销/沟通 UX；native mobile 为 post-Pilot 路线 | 当前内部 Pilot 的 authenticated Web 全旅程手动演示通过。 |
...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C decision complete; Pilot-0-D design locked at `DR-P0=0 / DR-P1=0 / DR-P2=0`; six `TR-P0` and three `TR-P1` remain open, with one accepted-scope `TR-P1`; no candidate/QR exists | X5 | Readiness only; separately authorized completed/qualified `platform_child_family_identity_source_v1` inside current C-3 -> strict C40–C45 -> D implementation and immutable complete candidate/evidence seal -> E decision. `db22de6` schema-only does not qualify; all implementation/deployment/activation remains separately authorized. |
...
dev-docs/active/nurture-institution-mode/
  ...
  10-pilot0-c-current-decision-index.md
  11-pilot0-d-topology-operations-contract.md
```
