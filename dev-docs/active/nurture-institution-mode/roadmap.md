...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2e-4a locks owner-only replacement, one-way lineage, atomic same-owner successor, old-work isolation, races, and explicit-empty output; C-2e-4b revoke next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 的 C-2e-4a 已锁定。Replacement 只允许 exact current owner 通过五分钟 strong old/new-delta review 执行；new Grant 以 unique `supersedesGrantId` 单向关联 old Grant，并继承同一 workspace/process/Enrollment/owner。一次 Serializable transaction 使用同一 database time 完成 old `replaced`、new `active`、Thread reuse、old-work fence 和 Execution，既无 active overlap 也无 gap。Same-definition 不续期，terminal old/owner loss/topology transfer 走其他路径，old objects 永久保留 old `grantId`，Thread reuse 不授予旧内容。Replacement 永远 explicit `[]` + null driver。C-2e-4b revoke、C-2e-4c owner loss、C-2e-4d cascade、C-2f、operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
