...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B and Pilot-0-C2 complete; C-3-0b authenticated ingress complete; C-3-0c-0/1/2 lock presentation ownership, provider wire, safe copy, semantic blocks, navigation/action offers, AI projection, privacy, and bounds; C-3-0c-3 is next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B、Pilot-0-C2 和 C-3-0b authenticated-ingress planning 已完成，C-3-0c-0/1/2 presentation ownership、subject-provider wire 与 semantic presentation 已锁定。Nurture 返回 owner-localized plain text、六类 flat blocks、独立 navigation/action offers 和 entitled unavailable disclosure；AI 只能接收剥离 refs/versions/codes/cursors/targets 后且显式允许 narration 的 safe text。`present_subject_context` 是 read-only，action offer 只能进入 C-3-0d prepare，不能授权 submit。以上仍是 planning-only。C-3-0c-3 renderer/persistence、C-3-0c-4 conformance/adoption、C-3-0d/e、Guardian/Caregiver IIB、C-4 Institution IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
