...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2 is complete and C-3-0b-3 now locks product/transition/runtime ingress, the exact caller-origin matrix, Notification and Step-bound durable mechanics, the prospective onboarding exception, and separate provisioning/operator-recovery channels; C-3-0b-4 next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 和 Pilot-0-C2 已完成。C-3-0b-3 在 C-3-0b-2 双重私有证明之上锁定了 versioned discriminated ingress：六个 Nurture product surfaces、`notification_open|invitation_continuation` Host transitions 和唯一 `workflow_worker` runtime ingress。API/interactive 与 worker/durable 的组合闭合；Notification 先做精确收件人 owner transition 再以新 destination invocation 打开；worker 从持久化 Run actor 与 claimed original Step 恢复且不伪装 UI surface；post-accept continuation 是唯一 invitation-bound Participant create/reuse 例外。C-0 provisioning 与 Technical Operator owner recovery 使用独立 endpoint/audience/caller/gate，不构造 ordinary human principal。以上仍是 planning-only，现有 broad `client_surface`、optional identity、event scaffold、owner-read token 和 mixed actor persistence 不可激活。C-3-0b-4、后续 Guardian/Caregiver IIB、C-4 Institution IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
