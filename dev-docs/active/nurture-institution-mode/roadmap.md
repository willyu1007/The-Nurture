...
| D8 | IIA implementation entry | **LOCKED DESIGN:** IIA-0 R1-R8、`NurtureInteractionContext`、Execution replay seed、non-empty -> persisted+claimed My-Chat Step and original-Step-exclusive recovery are closed；non-empty activation remains implementation-gated on host ledger/materializer | Historical drift scan + cross-repo implementation dependency map |
