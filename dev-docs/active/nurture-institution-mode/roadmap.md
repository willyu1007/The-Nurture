...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B and Pilot-0-C2 complete; C-3-0b authenticated ingress is complete through exact client echo, denial, audit retention/access, three-repo adoption, and negative closure; C-3-0c next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B、Pilot-0-C2 和 C-3-0b authenticated-ingress planning 已完成。C-3-0b-4 把 public input 固定为 discriminated `ScenarioClientEchoV1`，将 Host-established identity/route/request/driver/strong-auth evidence 与 Nurture-owner Participant/role/Subject/Grant/policy 分开；签名不提升 echo authority。Host ingress state、owner safe reason 与 HTTP/protocol failure 不形成第三套 reason 语义。审计保持 owner-split、body-free，并锁定 nonce 5 分钟、trace 7 天、log 14 天、aggregate/ordinary ingress audit 90 天、provisioning/operator/owner-recovery audit 365 天的 Pilot 默认；业务内容保留仍属后续隐私门禁。Base -> My-Chat -> Nurture 采用必须 additive、exact revision/hash、capability default-off、无 legacy fallback。以上仍是 planning-only，当前自由 payload/display state、optional identity、broad surface/error details、缺失 audit/retention/conformance 均是实现缺口。C-3-0c/d/e、Guardian/Caregiver IIB、C-4 Institution IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
