...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C locked through C-2b-2 Co-Guardian issue/acceptance; C-2b-3 rights/history next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 已锁定到 C-2b-2。任何当前 Guardian 均可发起准确 Family/Child/recipient 的 Co-Guardian Invitation；My-Chat 拥有联系方式、投递、认证和接受，Nurture 在接受时重查邀请人权限、对象生命周期、收件人和 Pilot 拓扑，再原子创建第二 Guardian 角色。发出邀请不产生家庭权限，待接受邀请可由准确邀请人取消，接受不转移 Grant 或创建 Enrollment。Pilot 只执行 Family-1 的一次邀请并保持 `2 + 1 + 1`；这不是 Schema 或产品人数上限。C-2b-3 rights/history、C-2b-4 exit/revoke、C-2c-f、后续 operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
