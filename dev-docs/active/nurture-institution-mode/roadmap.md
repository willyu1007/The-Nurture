...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B and Pilot-0-C2 complete; C-3-0b authenticated ingress complete; C-3-0c-0/1 lock presentation ownership plus subject-provider list/resolve, opaque ref, collection, result, privacy, and freshness rules; C-3-0c-2 is next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B、Pilot-0-C2 和 C-3-0b authenticated-ingress planning 已完成，C-3-0c-0/1 presentation pipeline/ownership 与 subject-provider wire 已锁定。客户端不能提供 `command_request_id`；My-Chat 只建立 request/driver 与 authentication-assurance evidence，Nurture 按 operation/driver contract 校验 canonical effect identity/hash/replay 与业务授权。subject-provider 使用独立 `list_subject_contexts`/`resolve_subject_context`、新 opaque `ScenarioSubjectContextRefV1`、collection-as-context、closed results 和精确隐私字段；ref/cursor 分别最长 30/5 分钟，单页最多 20 项。该 ref 不是关系、action、replay 或 offline authority。以上仍是 planning-only。C-3-0c-2 blocks/actions、C-3-0c-3 renderer/persistence、C-3-0c-4 conformance/adoption、C-3-0d/e、Guardian/Caregiver IIB、C-4 Institution IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
