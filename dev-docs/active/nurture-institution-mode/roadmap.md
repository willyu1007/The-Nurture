...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2d-1/2 atomic Enrollment and lifecycle/concurrency locked; C-2d-3 thread timing next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 已锁定到 C-2d-2。确认事务创建 `active` Enrollment，`joinedAt` 取同一原子事务内由数据库生成的权威 transaction timestamp；同一 ChildCareProcess 在同一 Institution 跨 CareGroup 只能有一个 pending/active/paused current Enrollment，不同 Institution 可独立存在且权限/Grant 不共享。同机构转班不自动发生，exact replay、新 command duplicate/conflict 和并发均确定性处理，loser invitation 不消费且无部分事实。ended/withdrawn 不复活，re-entry 使用新 roster/invitation/Enrollment；deleted 不是 Pilot 逃生口。C-2d-3 thread 时机、C-2d-4 result/Handoff、C-2e-f、后续 operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
