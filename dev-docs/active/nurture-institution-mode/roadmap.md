...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2d-1/2/3 Enrollment, lifecycle/concurrency, and first-Grant Thread timing locked; C-2d-4 result/Handoff next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 已锁定到 C-2d-3。Enrollment 不创建 Thread；机构/老师在 Grant 前只能 owner-read 安全 roster/enrollment 投影。首个 C-2e active Grant 与 exact enrollment-private Thread/Execution/audit 同事务创建，禁止 first-Message lazy create；不同 Institution Enrollment 使用不同 Thread，exact replay/Grant replacement 复用原 Thread。Thread/participant 从不授权，每次 read/write 重查 role、Enrollment/CareGroup、original/current Grant、policy、source/redaction。revoke/expiry 保留 Thread/audit 但 fail-closed，后续 Grant 不复活原 Grant 已失效内容；re-entry 使用新 Enrollment-scoped Thread。C-2d-4 result/recovery/Handoff、C-2e-f、后续 operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
