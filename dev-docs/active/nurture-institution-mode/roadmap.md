...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2f-3a locks separate Guardian withdrawal/Institution service-end actions, equal independent Guardian authority, fixed terminal statuses/reasons, strong confirmation, and active/paused terminal entry; C-2f-3b atomic closure next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 的 C-2f-3a 已锁定。Guardian 三端新增 `withdraw_family_enrollment -> nurture.family_care.withdraw_enrollment`；Institution workbench 保留 `close_enrollment -> nurture.institution.close_enrollment`，board read-only。Any current exact-Family Guardian 可独立 withdraw，不依赖 Grant owner、invitation recipient、first/primary Guardian 或 unanimous Co-Guardian；any current exact-scope Institution Admin 可独立 end service，双方不互相 countersign。五分钟 strong confirmation 与 server-fixed `withdrawn + family_withdrawn`、`ended + institution_service_ended`、既有 transfer `ended + care_group_transfer` 保持三类 terminal 语义分离。`active|paused` 均可进入 terminal path，hold 不得 veto，也不能伪装为 side release。C-2f-3b exact atomic hold/Grant/Thread/work closure、C-2f-3c fresh re-entry、C-2f-4/5、operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
