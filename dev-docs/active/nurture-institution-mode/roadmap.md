...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2f-4-1 locks versioned linear StageEpisode, equal Guardian strong-confirmation authority, stable coarse/allowlisted fine vocabulary, change/correction/clear/replay/races, projection-only `currentStageKey`, and zero Institution/inference/Enrollment side effect; C-2f-4-2 visibility/concurrency next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 的 C-2f-4-1 已锁定。Additive `NurtureChildCareStageEpisode` 是同 workspace ChildCareProcess 的线性 stage SSOT：versioned catalog、stable coarse key、optional allowlisted fine key、one current、unique predecessor、database-time start/end、exact Guardian/RoleAssignment/Execution audit。任一 current exact-Family Guardian 都可从三种 Guardian surface 通过五分钟 strong confirmation 使用唯一 `update_child_care_stage` action，独立执行 initial/change/correction/clear；不设置 primary/unanimity。Age/birthday/Institution/roster/AI/pregnancy-guidance handler 均不能提交 stage。`currentStageKey` 只投影 current Episode coarse key；mismatch fail-closed，legacy unproven value 不自动 backfill。Stage update 不改变 Enrollment、Grant、Thread 或历史 artifacts，也不自动暴露给 Institution。C-2f-4-2 multi-Institution visibility/concurrency、C-2f-4-3 future cross-workspace protocol boundary、C-2f-5 result/delivery、operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
