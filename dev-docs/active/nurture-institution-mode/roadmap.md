...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2d-1 atomic Enrollment core locked; C-2d-2 lifecycle/concurrency next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 已锁定到 C-2d-1。`confirm_family_enrollment` 只能由精确邀请收件人且仍为当前 Guardian 强确认；submit context、CommandExecution、Enrollment、RosterEntry-child link、邀请 consumed 与 audit/result refs 必须在一个 Nurture 事务中全成或全退，response loss 重放原结果，且不隐式创建 Grant 或老师家庭内容权限。初始 Enrollment 状态/时间和生命周期/并发留给 C-2d-2，thread 时机留给 C-2d-3，result/Handoff 留给 C-2d-4。C-2d-2-f、后续 operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
