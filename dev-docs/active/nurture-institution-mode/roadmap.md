...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2f-4-0 locks longitudinal-process/stage/Institution-Enrollment/workspace classification, independent different-Institution onboarding, no cross-scope lineage or authority carryover, Enrollment-independent stage change, and cross-workspace `NO-GO`; C-2f-4-1 stage lifecycle next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 的 C-2f-4-0 已锁定。`NurtureChildCareProcess` 是同 workspace 内由当前 Family owner 解析的长期主轴；stage 是独立的 family-owned phase；Enrollment 是 Institution-local relationship episode。不同 Institution 必须走 fresh roster/invitation/Guardian confirmation，可复用 family-selected Child/Process，但可与旧 Institution Enrollment 并存，不自动退出、不建立 predecessor lineage，也不继承 Grant/Thread/content/role/authority。Stage change 不创建、迁移或终止 Enrollment；stage 与新 Institution 同时变化时是两个独立授权效果，不做跨域原子事务。Cross-workspace profile reuse、fuzzy/global identity inference、raw linking 和 automatic migration 在 Pilot 中均为 `NO-GO`。C-2f-4-1 必须进一步锁定 versioned stage fact、Guardian authority、transition/correction/history 和 `currentStageKey` projection；C-2f-5 result/delivery、operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
