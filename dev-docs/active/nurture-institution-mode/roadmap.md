...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2f-1 locks side-owned pause holds, compatible family/Institution actions, same-side shared authority/cross-side denial, strong confirmation, atomic aggregate/replay/races, preserved facts/clocks, and no cascade/bulk replay; C-2f-2 transfer next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 的 C-2f-1 已锁定。Additive `NurtureEnrollmentPauseHold` 每个 Enrollment/side 最多一个 active family/institution hold，使用 `active|released|closed`、fixed safe reason、exact actor/RoleAssignment/Execution/database time，无 free text/auto-expiry。Hold 是 restriction authority，Enrollment status 是 atomic aggregate，mismatch fail-closed。Any current exact-family Guardian 共享 family-side 操作，exact-scope Institution Admin 共享 institution-side 操作，cross-side release 禁止。Guardian 使用 `suspend_family_enrollment` / `resume_family_enrollment`；Institution 复用已锁定 keys，resume 只 release institution hold。双方使用五分钟 current confirmation。Serializable transaction 使 replay stable、duplicate already-satisfied、stale cross-side concurrency refresh/reconfirm。Pause 保留 Grant/work lifecycle 和 clocks，阻断 current cross-role use，使旧 contexts 失效，不 cascade、不 bulk replay，也不把 Institution/CareGroup pause fan-out 为 holds。C-2f-2 transfer、C-2f-3..5、operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
