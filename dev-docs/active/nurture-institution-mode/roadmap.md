...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2e-4b locks exact-owner revoke, strong confirmation, server audit/database time, atomic fence, replay/races, equal-Guardian fresh authorization, and explicit-empty output; C-2e-4c owner loss next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 的 C-2e-4b 已锁定。Voluntary revoke 只允许 exact current owner/current Guardian 通过五分钟 strong confirmation 执行；一次 Serializable transaction 使用 database time 和 server-owned `user_revoked` 完成 `active -> revoked`、完整同事务 fence 和 Execution，并只保存 terminal Grant/Thread refs。Exact replay 固定原结果，新 eligible revoke 对同一 revoked Grant 为 `already_satisfied` 且不重写 audit；其他 terminal 状态不是成功，races first-commit-win。Revoked Grant 不可恢复，old work 永不复活，但任何 current equal Guardian 都可通过完整 fresh confirmation 建立 future-only new Grant。Revoke 永远 explicit `[]` + null driver。C-2e-4c owner loss、C-2e-4d cascade、C-2f、operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
