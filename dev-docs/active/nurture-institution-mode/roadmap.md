...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2e-4c locks exact owner-role binding, Host/suspension/terminal loss, self-exit/local convergence, no transfer/new-role revival, fresh future authorization, and explicit-empty output; C-2e-4d cascade next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 的 C-2e-4c 已锁定。Grant 除 owner Participant 外还必须 additive 绑定 exact `grantedByRoleAssignmentId`；new role row 不得复活 old Grant。Host loss 只阻断该用户访问且不改 Nurture，exact role suspension 暂时阻断 Grant 并仅可由 same row 恢复，terminal role loss 永久 fence 且不转移 owner。Self-exit 原子提交 role/owned-Grant/fence/Execution；其他 terminal loss 立即 fail-closed 并由 Nurture-local owner action 收敛。另一 current Guardian 只能通过完整 fresh confirmation 创建 independent future-only Grant，无 `supersedesGrantId`，Thread reuse 不恢复旧工作。所有 owner-loss paths 永远 explicit `[]` + null driver。C-2e-4d cascade、C-2f、operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
