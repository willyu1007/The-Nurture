...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2 is complete and C-3-0b-1 now separates platform-general/workspace-business/invitation contexts, requires explicit general-to-private transition, fixes workspace source/session/gate semantics, and keeps Host acceptance independent from Nurture onboarding; C-3-0b-2 next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 和 Pilot-0-C2 已完成。C-3-0b-1 将 My-Chat public context 固定为 `platform_general|workspace_business|invitation_acceptance`。通用 Chat 即使以 personal workspace 存储也不获得 business context；涉及私有家庭/孩子/老师/机构事实时必须显式转入 workspace-scoped conversation，默认只携带当前确认意图。业务 workspace 只来自 server-owned resource、explicit shell selection 或已明确进入业务 route 后的 sole membership；session、membership、capability、scenario、allowlist/cohort 和 surface 依序重检。Host Invitation 可在没有 target membership 时显式接受，但 membership commit 与 Nurture onboarding 独立且不会自动执行业务。C-3-0b-2 private service/principal envelope、后续 Guardian/Caregiver IIB、C-4 Institution IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
