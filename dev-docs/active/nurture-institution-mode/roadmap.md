...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B and Pilot-0-C2 complete; C-3-0b authenticated ingress complete; C-3-0c-0/1/2/3 lock presentation ownership, provider wire, semantic output, renderer mapping, four persistence classes, freshness, stale behavior, and accessibility; C-3-0c-4 is next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B、Pilot-0-C2 和 C-3-0b authenticated-ingress planning 已完成，C-3-0c-0/1/2/3 presentation ownership、subject-provider wire、semantic presentation、renderer mapping、persistence/freshness/accessibility 已锁定。六个产品 surface 只映射到 My-Chat 通用 Chat/role-board/domain-workbench renderer；Institution board 只读，Teacher board 保留完整 owner 分页历史，任意搜索/复合过滤延后到独立 owner query-control 合同。presentation 默认 ephemeral，Host 仅能持久化 content-free shell/rehydration marker，并使用最多 60 秒前台 freshness lease 与 fail-closed stale 行为。以上仍是 planning-only。C-3-0c-4 conformance/adoption、C-3-0d/e、Guardian/Caregiver IIB、C-4 Institution IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
