...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A complete; B1/B2/B3-0/B3-1, B3-2a-d, and B3-3a-c locked; B3-3d/B3-4 remain open | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5 与 Pilot-0-A 已完成；revised Pilot-0-B1/B2、B3-0/B3-1、B3-2a-d 以及 B3-3a-c 已锁定。B3-3a 固定首个输入/数据包络，B3-3b 固定单一双向 Grant 和家庭权限，B3-3c 固定 atomic capture/acknowledge/reply、Receipt 逻辑分发语义和 `replied` Pilot 终态。B3-3d failure/privacy、B3-4 journey coverage、real cohort sizing、IIB/onboarding、isolated topology、workspace allowlist、成功/停止和回滚仍待逐条锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
