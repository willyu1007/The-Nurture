...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2e-2 locks active Grant identity/partial uniqueness, one serializable Grant/Thread transaction, database-time expiry, replay/race convergence, and bounded retry; C-2e-3 result/recovery/Handoff next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 的 C-2e-2 已锁定。Active Grant 由 exact workspace/process/Enrollment/CareGroup/canonical-purpose identity 和 raw partial unique index 保护；directions/data classes 是 canonical profile，变化必须走 replacement。一次 Serializable Nurture transaction 使用 database transaction time 完成 replay/context/current-fact classification、Grant、exact Enrollment Thread 和 Execution/audit refs。Exact replay 不重复写；不同 command id 的并发由 first commit 获胜，loser 在 bounded whole-transaction retry 后记录 `already_satisfied`，不转移 owner 或延长 expiry。Owner reads 使用 `[effectiveFrom, expiresAt)` 实时 fail-closed，无需 expiry worker。C-2e-3 result/recovery/explicit-empty Handoff、C-2e-4 replacement/revoke/owner loss、C-2f、operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
