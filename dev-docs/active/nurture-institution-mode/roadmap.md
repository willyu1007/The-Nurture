...
| Pilot-0 | The-Nurture governance + cross-repo review | Pilot-0-A/B complete; Pilot-0-C C-2c-1/2 issue and child-branch locked; C-2c-3 invitation lifecycle next | X5 | Readiness only; external traffic NO-GO; no DB, artifact publication, secret, capability, or traffic change |
...
当前阶段为 Pilot-0 readiness：X5、Pilot-0-A/B 已完成，Pilot-0-C 已锁定到 C-2c-2。Host acceptance 只证明准确收件人；当前 Guardian 必须用 Nurture owner-resolved opaque option 明确选择同 workspace child process，无 Guardian 关系的收件人用独立 C-2b-1 命令原子建档。现有档案不能由非 Guardian 认领，机构预填不匹配/关联孩子。待选择只在短期 InteractionContext 中，C-2d 前不写 roster link/Enrollment/Grant/thread/teacher fact，也不消费邀请。新建家庭档案即使邀请后续取消/过期仍作为独立家庭事实保留。C-2c-3/4、C-2d-f、后续 operational IIB、topology/operations 和 Go/No-Go 仍待锁定。Pilot-1..4、live traffic、数据库 apply、ACR 发布、secret 配置、capability activation、staging、production 和 GA 均未授权。
