...
Pilot-0-C2f-4-3 keeps every cross-workspace lookup, fuzzy/global match, reuse, link, import, and protocol route disabled in Pilot while defining the minimum future boundary. A first protocol version is a copy-and-reconfirm flow only for the same My-Chat adult authenticated in both workspaces: a current exact-source-Family Guardian issues, and the target adult reauthenticates and strongly confirms a new target-local Child/ChildCareProcess/Family/first-Guardian aggregate. Source authority never crosses; Nurture stores only opaque Host recipient binding while My-Chat owns raw account identity/authentication and refs-only delivery. The v1 payload is limited to Guardian-confirmed child `displayName` and separately selected optional `birthDate`; StageEpisode/history, relationship metadata, Institution/Roster/Enrollment, Grant/Thread/content, media/health/pregnancy, contacts, audit, raw ids, and technical state are forbidden. Existing target profiles are not searched, suggested, merged, overwritten, or linked; reconciliation remains a later target-local protocol. A future Nurture-owned portability ledger must bind protocol version, source scope/actor, exact target workspace/opaque recipient, field allowlist/hash, seven-day expiry, lifecycle, replay, and audit; final confirmation uses a five-minute target context. Any current exact-source-Family Guardian may revoke a pending Intent, and current source/target authority, source field lifecycle/redaction, expiry, and policy are reread until the single consumption decision. Exact replay recovers one target result; revoke-versus-consume is linearized; post-consume source revocation cannot distributed-delete target-local facts. Existing Workflow Handoff/Outbox cannot carry portability bodies or become the business ledger. C-2f-5 owns exact result, surface, recovery, and Host-effect semantics outside this planning-only boundary.
