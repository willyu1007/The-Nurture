...
| Pilot-0-C IIB/onboarding contract | In progress — C-2e complete | C-2e-4d completes permanent-versus-temporary invalidation, typed context dependencies, exact receipt/item/attention/Thread/redaction matrices, one root-first keyset loop-to-closure transaction, bounded audit/overflow, stale Host delivery, and explicit-empty effects. C-2f leave/transfer/next-stage is next. |
