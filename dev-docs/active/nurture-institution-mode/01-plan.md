...
| Pilot-0-C IIB/onboarding contract | In progress — C-0/C-1/C-2a locked | C-0 fixes authenticated ingress/bootstrap. C-1 fixes the sole CareGroup, derived readiness, staff acceptance and separate roles. C-2a adds a design-only, institution-local roster entry for families without an existing product profile while reserving longitudinal child-process creation/selection, Enrollment confirmation, and Grant to authenticated Guardian authority. C-2b Family/Co-Guardian Invitation is next. |
