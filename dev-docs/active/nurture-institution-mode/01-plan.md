...
| Pilot-0-C IIB/onboarding contract | In progress — C-2c-1/2 locked | C-2c-2 treats Host acceptance as identity evidence only. Current Guardians explicitly choose an owner-resolved same-workspace process via opaque options; recipients without a Guardian relationship use atomic C-2b-1 profile creation. Pending choice stays in InteractionContext, writes no Roster link/Enrollment/Grant/thread, and the invitation remains pending until C-2d commit. C-2c-3 lifecycle is next. |
