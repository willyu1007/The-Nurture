...
| Pilot-0-C IIB/onboarding contract | In progress — C-3-0a locked | C-3-0a makes every activated education/nurture business surface account-reachable through a scenario-owned relationship path, uses generic `subject` plus `unresolved|single_subject|subject_collection`, keeps My-Chat subject entries opaque/non-authoritative, and forbids implicit global/cross-workspace/cross-scenario identity. C-3-0b authenticated ingress/trusted principal is next. |
