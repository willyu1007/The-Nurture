...
| Pilot-0-C IIB/onboarding contract | In progress — C-2b complete | C-2b-4 permits only strongly confirmed self-exit when another current Guardian remains; peer, Institution, Caregiver, and Operator removal is forbidden. One transaction revokes the exiting role, pending invitations issued by that actor, and active Grants owned by that actor with normal cascades. Access stops immediately, facts/audit remain, and rejoin requires a new invitation/role. C-2c Enrollment Invitation is next. |
