...
| Pilot-0-C IIB/onboarding contract | In progress — C-2e-2 locked | C-2e-2 fixes the active Grant identity and raw partial uniqueness, canonical same-definition comparison, one serializable Grant/Thread/context/Execution transaction, database-time expiry, exact replay, first-commit-wins `already_satisfied`, bounded retry, and integrity-conflict behavior. C-2e-3 result/recovery/explicit-empty Handoff is next. |
