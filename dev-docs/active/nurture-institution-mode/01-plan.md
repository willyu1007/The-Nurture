...
| Pilot-0-C IIB/onboarding contract | In progress — through C-2b-1 locked | C-2a separates institution-local intake from the longitudinal profile. C-2b-1 requires exact My-Chat authentication plus strong relationship/profile confirmation and one atomic Participant/Child/Process/Family/first-Guardian transaction; Institution invitation alone proves no relationship, existing profiles require current Guardian authority, and no `primary_guardian` hierarchy is added. C-2b-2 Co-Guardian Invitation is next. |
