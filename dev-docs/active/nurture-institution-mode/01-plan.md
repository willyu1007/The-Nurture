...
| Pilot-0-C IIB/onboarding contract | In progress — C-2f-3b locked | C-2f-3b unifies Enrollment-first topology locking, exact terminal audit and server invalidation causes, system-closed holds, invalidated transfer intents, typed Enrollment context dependencies, aggregate hard-cap preflight, atomic Grant/dependent/Thread/roster closure, survivor assertions, replay/races, and whole rollback. C-2f-3c fresh re-entry/history is next. |
