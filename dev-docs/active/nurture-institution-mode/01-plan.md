...
| Pilot-0-C IIB/onboarding contract | In progress — C-2f-1 locked | C-2f-1 adds side-owned pause holds, preserves existing Institution keys while adding distinct family keys, requires current strong confirmation, makes resume side-local rather than globally active, keeps Grant/work facts and clocks intact behind current fences, and serializes replay/races without stale-context silent merge. C-2f-2 transfer is next. |
