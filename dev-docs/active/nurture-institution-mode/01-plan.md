...
| Pilot-0-C IIB/onboarding contract | In progress — C-2f-0 locked | C-2f-0 separates current `active|paused` from terminal `ended|withdrawn`, reserves `deleted`, assigns family-side restriction/withdrawal to Guardian and institution-side restriction/end/transfer proposal to Institution Admin, denies Caregiver/Operator/AI topology authority, classifies pause as reversible without permanent cascade, and forbids in-place transfer or automatic cross-scope migration. C-2f-1 pause/resume is next. |
