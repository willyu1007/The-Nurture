...
| Pilot-0-C IIB/onboarding contract | In progress — C-2e-4c locked | C-2e-4c binds Grant ownership to the exact Guardian RoleAssignment, separates Host loss from resumable Nurture suspension and terminal role loss, atomically composes self-exit, defines local convergence and fresh equal-Guardian authorization without transfer/revival, and keeps every owner-loss path explicit-empty. C-2e-4d cascade closure is next. |
