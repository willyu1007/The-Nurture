...
| Pilot-0-C IIB/onboarding contract | In progress — C-2e-1 locked | C-2e-1 permits any current exact-family Guardian to review/confirm, makes the first committed confirmer the sole Grant owner, requires common strong-confirmation disclosure plus a five-minute actor/surface-bound submit context, and classifies existing Grant/current-state results without ownership transfer. C-2e-2 atomic identity/time/expiry/concurrency is next. |
