...
| Pilot-0-C IIB/onboarding contract | In progress — C-0/C-1 locked | C-0 fixes authenticated ingress/bootstrap. C-1 uses `NurtureCareGroup` as the sole class object; locks Institution-workbench lifecycle commands, derived lead-caregiver/policy readiness, Staff Invitation → My-Chat acceptance → Participant binding → separate Caregiver/Lead RoleAssignment, and fail-closed revoke/offboarding. C-2 family/child/enrollment/Grant onboarding is next. |
