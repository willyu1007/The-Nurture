...
| Pilot-0-C IIB/onboarding contract | In progress — C-2f-4-2 locked | C-2f-4-2 permits a current-Guardian longitudinal family aggregation but enforces repository-level Institution/CareGroup partitioning, no other-Institution existence/id leakage, no implicit stage visibility/dataClass, independent per-Institution lifecycle/concurrency, exact dependency rereads, and safe stale-episode degradation. C-2f-4-3 future cross-workspace protocol boundary is next. |
