...
| Pilot-0-C IIB/onboarding contract | In progress — C-3-0b-0 locked | C-3-0b-0 fixes the two-proof ingress model: My-Chat service identity authenticates the private call, an exact Host-established adult principal identifies account/actor/workspace/scenario/surface, and Nurture alone resolves Participant/role/Subject/action authority. Invitation acceptance remains a separate identity transition; legacy optional actor/workspace inference cannot activate. C-3-0b-1 public ingress/session/workspace is next. |
