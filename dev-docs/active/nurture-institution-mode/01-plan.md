...
| Pilot-0-C IIB/onboarding contract | In progress — C-3-0b-3 locked | C-3-0b-3 fixes registered product/transition/runtime ingress classes, exact API/session and worker/Run-actor combinations, two-stage Notification open, Step-bound durable replay, the sole invitation-bound prospective Participant exception, and separate provisioning/operator-recovery channels. C-3-0b-4 client echo, safe denial, audit, adoption, and negative closure evidence is next. |
