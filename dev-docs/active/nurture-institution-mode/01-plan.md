...
| Pilot-0-C IIB/onboarding contract | In progress — C-2f-4-1 locked | C-2f-4-1 makes versioned linear StageEpisode the family-owned SSOT, gives every current exact-Family Guardian equal strong-confirmation authority, fixes coarse/fine vocabulary, transition/correction/clear/replay/race semantics, keeps `currentStageKey` projection-only, and forbids Institution/inference/Enrollment side effects. C-2f-4-2 multi-Institution visibility/concurrency is next. |
