...
| Pilot-0-C IIB/onboarding contract | In progress — C-3-0b-2 locked | C-3-0b-2 requires independent workload authentication and a signed/request-bound human-principal envelope; fixes exact User/Actor/Workspace/origin semantics, 60-second freshness, nonce-versus-command replay, audience/caller/key rotation, strict private codec, normalized verification boundary, and additive no-fallback activation. C-3-0b-3 ingress variants are next. |
