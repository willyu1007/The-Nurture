...
| Pilot-0-C IIB/onboarding contract | In progress — C-3-0b-1 locked | C-3-0b-1 separates `platform_general`, `workspace_business`, and `invitation_acceptance`; forbids treating personal storage as business context or silently upgrading general Chat; fixes exact workspace sources, session/membership/gate ordering, minimum invitation acceptance, separate Nurture continuation, safe public states, and current implementation gaps. C-3-0b-2 private caller/principal envelope is next. |
