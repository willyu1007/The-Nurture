...
| Pilot-0-C IIB/onboarding contract | In progress — C-2e-3 locked | C-2e-3 separates immutable command receipt from current owner presentation, fixes exact Grant/Thread refs and safe result/actor-relation vocabulary, recovers response loss before consumed-context checks, forbids result tokens and raw refs, and keeps confirmation explicit-empty with no host activation or protected-content effect. C-2e-4 replace/revoke/owner-loss/cascade is next. |
