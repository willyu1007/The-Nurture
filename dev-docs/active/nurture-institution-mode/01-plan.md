...
| Resolve service-framework timing | In progress — M0/M1 complete | The formal NestJS scenario-service skeleton now builds, starts, exposes only health plus a fail-closed binding-owner stub, and has its own CI verification job. M2 auth, M3 P7 migration, M4 governance alignment and M5 handoff regeneration remain; the Fastify harness is not promoted. |
