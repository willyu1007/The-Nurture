...
| Pilot-0-C IIB/onboarding contract | In progress — C-2c-1 locked | C-2c-1 restricts issue to current Institution Admin plus full C-1 readiness and exact Institution/CareGroup/unlinked-RosterEntry/Host-recipient/version/expiry/hash binding. Nurture intent is the business fence, one effective pending intent exists per RosterEntry, and issue creates no participant/child/family/Guardian/Enrollment/Grant/thread fact. C-2c-2 acceptance branching is next. |
