...
| Pilot-0-C IIB/onboarding contract | In progress — C-2f-4-3 locked | C-2f-4-3 keeps Pilot cross-workspace use `NO-GO` while fixing a future same-adult, dual-owner-read, fresh-target-identity export/import boundary; minimum explicit profile fields, no stage/Institution/authority/content carryover, no match/merge, seven-day target-bound Intent, five-minute confirmation, pending revoke/expiry/redaction denial, exact-once target recovery, and no post-import source recall. C-2f-5 result/recovery/surfaces/Handoff is next. |
