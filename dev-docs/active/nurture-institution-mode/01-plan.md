...
| Pilot-0-C IIB/onboarding contract | In progress — C-2f-3a locked | C-2f-3a separates Guardian withdrawal from Institution service end, fixes action/command and terminal status/reason mappings, gives every current Guardian equal independent withdrawal authority, rejects peer/Grant-owner/primary/unanimous/technical substitutes, and permits terminal exit from active or paused without treating holds as releases. C-2f-3b atomic terminal closure is next. |
