...
| Resolve service-framework timing | Decomposed, pending implementation | Record and implement the approved transition from the current Fastify dev-host harness to the required NestJS scenario service without promoting the harness to a pilot runtime. The accepted M0-M5 decomposition and ING-D1..D7 decisions live in `12-nestjs-ingress-migration-plan.md`. |
