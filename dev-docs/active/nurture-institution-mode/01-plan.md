...
| Pilot-0-C IIB/onboarding contract | In progress — C-2f complete | C-2f-5 closes lifecycle planning with typed business outcome versus replay disposition, exact server-side output refs, current owner presenters, same-Execution/same-Step response-loss recovery, route-only cross-surface continuity, and a minimal additive Guardian relationship-attention contract that does not widen existing `user_attention` or earlier explicit-empty decisions. C-3 Guardian/Caregiver operational IIB is next. |
