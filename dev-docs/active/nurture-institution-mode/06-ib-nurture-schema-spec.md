...
- Existing `currentStageKey` remains an optional legacy/current projection and is not promoted to mutable historical authority by this checkpoint. C-2f-4-1 must lock the versioned stage fact, exact Guardian authority, transition/correction/history semantics, and projection derivation before schema or command implementation.
- C-2f-4-0 remains planning-only and adds no schema, action, handler, runtime, database, Handoff, notification, or traffic effect. C-2f-4-1 owns stage lifecycle; later C-2f-4 checkpoints own remaining cross-Institution privacy and future-protocol detail; C-2f-5 owns result/recovery/presenter/Handoff semantics.
