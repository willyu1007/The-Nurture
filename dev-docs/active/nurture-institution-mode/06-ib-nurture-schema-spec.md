...
- The principal contains no email/contact/display PII, membership id/role, Participant/RoleAssignment, Nurture role, Subject/Family/Child/Process, Institution/CareGroup/Enrollment, Grant/policy/action/target, surface, payload, session, invitation, scenario, service, or claim credential. Zero/duplicate/mismatched Participant fails closed without alternate-workspace, email/contact, history, cache, invitation, token, or client fallback.
