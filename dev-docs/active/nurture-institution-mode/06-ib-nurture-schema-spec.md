...
- Add unique nullable Restrict `supersedesEnrollmentId` from new Enrollment to old Enrollment. No old-row successor field is stored. Lineage requires same workspace/ChildCareProcess/Institution, different CareGroup, current source, exactly one direct successor, and no cycle/cross-scope link. Broken/ambiguous legacy topology blocks transfer; no auto-link.
- Old Enrollment uses existing `ended`, not a new transferred status. Add server-owned terminal reason and terminal CommandExecution ref: transfer writes `status=ended`, `leftAt=T`, `terminalReason=care_group_transfer`. New Enrollment writes `status=active`, `joinedAt=T`, `supersedesEnrollmentId=old.id`, target Group/roster, and no hold. One database transaction time provides no observable overlap or gap.
...
- One Serializable transaction resolves replay; locks intent, Guardian role/Family/process, source Enrollment, Groups, holds, source roster, then Grant/Thread/cascade roots; rechecks readiness/capacity/uniqueness/policy; preflights C-2e hard cap; obtains database `T`; ends old Enrollment; terminalizes every old effective Grant with server cause `enrollment_transferred`; completes all C-2e dependents; archives old Thread/source roster; creates target roster/new Enrollment; consumes intent; and persists lineage/audit/Execution. No Host/remote call occurs inside.
