...
3. **Child-centered:** `NurtureChildCareProcess` is the center. Families join the
   process；teachers include one or more such scopes in class workflows；institution
   managers govern how these scopes are organized.
...
- `workspaceId` is a scenario shell/runtime partition reference supplied by
  My-Chat. The partition reference does not make My-Chat the owner of Nurture role
  or care facts.
...
- Organization-level configuration, such as institution profile, care group rhythm,
  and teacher assignment, may exist without `childCareProcessId`. Once a record
  describes an individual child's care, communication, observation, media exposure,
  or delivery state, the record must include `childCareProcessId`.
...
- The Pilot records a product assertion and audit result, not legal guardian
  verification. Identity documents, civil-status evidence, institution attestation,
  and offline review are outside the product-assertion command and require a
  separate sensitive-data contract before real use.
...
- My-Chat owns raw contact, provider delivery, recipient authentication, invitation
  acceptance, and exact recipient Workspace plus Family membership. A future Nurture
  invitation-intent record owns only opaque Host invitation ref, inviter/family/
  process binding, suggested relationship metadata, expiry/version/canonical payload
  hash, lifecycle/audit, and current-policy evidence；the current Prisma schema does
  not yet implement the invitation intent.
...
- Pilot policy permits one Co-Guardian acceptance for Family-1 and none for
  Family-2/Family-3, producing the exact `2 + 1 + 1` topology. The exact
  `2 + 1 + 1` limit is a cohort gate, not a unique constraint, Schema cardinality,
  or product maximum；later multi-Guardian policy remains separate.
...
- A new `already_satisfied` Execution records the requesting Guardian as business actor but points to the winning Grant/Thread. Current presentation may say that authorization already exists and that the actor can use the authorization, but cannot say that the actor confirmed, owns, jointly approved, or may administer the winning Grant.
...
Pilot-0-C3-0c-1 subject-provider wire schema refinement:

- Base adds a reusable `ScenarioSubjectContextRefV1 = string` wire type and MUST NOT reuse `DomainContextRef`. The codec enforces a bounded opaque value; only Nurture can decode or correlate the owner locator. Nurture binds Workspace, scenario, provider, current verified principal/Participant, owner locator, scope kind, opaque context version, issue time, and expiry. A ref is invalid across User, Workspace, or scenario boundaries.
...
- Current Base `WorkflowActionAvailability` remains Run-level and raw-target-shaped; current My-Chat mobile `InteractionEnvelope` contains broad `params|extensions|serverAction`; current Nurture surface refs encode database ids. None satisfies C-3-0c-2. The checkpoint is planning-only and changes no contract package, manifest, source, schema, migration, route, UI, renderer, runtime, cache, secret, environment, capability, provider, database, or traffic.
...
The persistence classification is exact:

| Class | Classification rule |
| --- | --- |
| `owner_canonical` | Nurture business facts, lifecycle, `CommandExecution`, and history are durable only under Nurture business retention. |
| `ephemeral_presentation` | Every C-3-0c-1/2 result, safe text, block/offer, ref/version/cursor, AI projection, selection/scroll/expanded state, and render tree lives only in current foreground/request memory. |
...
Freshness and accessibility are renderer compatibility rules:

- The Pilot foreground presentation freshness lease is at most 60 seconds and is shortened by any earlier owner-ref expiry. Known revoke/redaction/policy/lifecycle invalidation clears presentation immediately. Background/lock, sign-out, Workspace/account/thread/surface/process/renderer change clears every ephemeral field.
- Resume/focus, refresh, navigation, pagination, detail open, action prepare/result, retry, and Notification destination repeat Host gates plus current owner read. Expiry/invalidation removes owner content and disables controls before fetch. Owner/provider/database outage returns only the generic safe retry/unavailable shell; stale-while-revalidate, content cache, legacy/surface fallback, offline fallback, and optimistic Nurture business state are forbidden.
- My-Chat applies the owner locale/text direction and preserves semantic order/roles. Accessible output pairs label/value, never relies on tone/color/icon/order/position alone, and gives each control a safe name/role/state, focus/keyboard path, touch target, font scaling, and reduced-motion behavior. Unavailable actions expose only owner-approved reasons; omitted actions leave no assistive trace.
- Refresh/context-change/unavailable/completed Host transitions focus a generic status container and use generic polite announcement. Host code cannot infer urgent/assertive/emergency semantics from `critical` tone or expose `display_only`/hidden data to assistive technology.
- Current My-Chat Chat interaction state is transient but the wire is broad; persisted Chat messages lack semantic-placeholder separation; Workflow dashboard code branches on raw Run actions/targets/reasons; role boards/workbenches and the renderer registry/freshness/persistence scanner are absent; accessibility is partial. Nurture has no activated semantic presenter. None is C-3-0c-3 evidence.
- C-3-0c-3 is planning-only. No contract package, manifest, source, schema, migration, route, UI, renderer, runtime, cache, secret, environment, capability, provider, database, or traffic changed. The following C-3-0c-4 refinement now locks conformance/adoption evidence.

Pilot-0-C3-0c-4 conformance and adoption refinement:
...
- The additive shared contract is `ScenarioDomainActionContractV1`. One entry binds exact `(scenario_key, action_key)`, contract version, input schema/version, target-ref class, `explicit|strong_authorization`, entitled product surfaces, authenticated handler, Nurture command contract, and exactly one `scenario_direct_empty_v1|workflow_claimed_step_v1` driver. Driver selection is static and MUST NOT vary by surface, recipient count, `applied|already_satisfied`, `executed|replayed`, owner outage, or provider state. The atomic capability is `scenario_domain_action_execution_v1` and requires `trusted_scenario_invocation_v1` plus `scenario_subject_presentation_v1`.
- The versioned operations are `prepare_domain_action` and `submit_domain_action`. Prepare accepts the registered action key, owner-issued prepare target, optional opaque expected version, and exact registered input. It repeats current Host and owner resolution, then issues one five-minute `submit_action` InteractionContext containing the canonical owner-side action state. Prepare creates no CommandExecution, business mutation, Workflow Step, Handoff, Outbox, Notification, or provider effect. Draft/body transport, retention, encryption, cache, and offline behavior remain C-3-0e.
- Submit client echo contains only the owner-issued submit token, exact `confirmation=confirmed`, and bounded `client_mutation_id`. Action, target, version, input, Participant, role, scope, driver, command identity, Step, draft, snapshot, or claim evidence cannot be repeated or overridden. A strong action additionally binds current My-Chat authentication-assurance evidence; the evidence proves only the recent ceremony and never substitutes for Nurture current role/policy/target/context checks or the explicit confirmation.
- A direct context transitions from active to consumed only in the business transaction. A claimed context remains active with an immutable `boundWorkflowStepRef`; exact rebind to the same Step is idempotent and any other Step conflicts. Binding validates the still-current token/confirmation/assurance and persisted Host Step assertion but consumes no context, commits no business fact, and does not extend the original expiry. The claimed business transaction consumes the context only after the exact currently claimed original Step passes driver validation.
- `scenario_direct_empty_v1` creates no Run/Step and atomically commits context consumption, owner preconditions, business effect, typed CommandExecution, audit, exact output refs, `handoffRequestSnapshotsPayload=[]`, and `handoffDriverRef=NULL`. My-Chat may render the current owner result but cannot synthesize an Outbox, Notification, Handoff, or optimistic Nurture state.
- `workflow_claimed_step_v1` requires My-Chat to idempotently persist a content-free original Step under `client_mutation_id` in a non-claimable `awaiting_scenario_binding` state. The Step may retain only Host Workspace/User/Actor provenance, scenario/action/handler/contract identity, technical state/version, and safe request correlation. Raw submit token, owner target/version, Nurture input/body, InteractionContext id, claim token, Handoff draft, or inferred business state is forbidden. After Nurture immutably binds the exact Step, My-Chat may make it claimable. The worker supplies the transient claimed driver; Nurture atomically persists the effect, Execution, and snapshots; My-Chat atomically completes Step/Handoff/Outbox under the existing `complete_step` contract.
- A persisted unbound Step never becomes claimable and is stopped after the original context expires. A bound-but-not-published Step is recoverable by a bodyless exact Step-binding lookup and needs no raw token. First business execution after expiry fails closed and requires a new context/Step. If Nurture committed before expiry, exact Execution lookup precedes consumed/expired rejection and same-Step reclaim remains valid after expiry. Claim token and expected Step version are transient, may rotate on same-Step reclaim, and never enter Nurture persistence, identity/hash, logs, traces, metrics, Handoff, or Outbox.
- Direct canonical effect identity is derived by Nurture as `H("nurture.domain-action.direct.v1", workspace_id, scenario_key, action_key, interaction_context_id)`. Claimed identity is `H("nurture.domain-action.claimed-step.v1", workspace_id, scenario_key, original_step_id, action_key)`. The client cannot author either value. Canonical payload hash includes command key/version, typed business actor, owner scope/target/expected versions, canonical input, InteractionContext, driver, and original Step for claimed execution. Surface, `client_mutation_id`, transport nonce, correlation/trace, claim token, expected Step version, lease, attempt, and presenter state are excluded.
- Additive typed actor fields are required before activated C3 writes: `businessActorSchemaVersion=1`, `businessActorKind=nurture_participant`, and Restrict FK `businessActorParticipantId`. Existing `business_actor_ref` remains unchanged legacy historical evidence and MUST NOT be backfilled or interpreted globally as Participant or Host Actor. My-Chat User/Actor/Workspace provenance uses separately named bounded audit fields/hash and never becomes the Nurture business actor. Provisioning, owner-recovery, service, and Technical Operator paths use their own classified contracts and cannot populate the ordinary Participant actor path.
- Direct transaction order is exact replay lookup, context/actor/owner-root locks, current preconditions, database time, context consume, business mutation, Execution/audit/output refs/explicit-empty persistence, and commit. Claimed order adds immutable Step binding plus current claim validation before owner locks and persists exact snapshots in the same transaction. InteractionContext consumption MUST NOT remain a separate repository commit before command execution. Remote Host/provider calls occur outside both Nurture transactions.
...
- Add the ordinary self-exit mapping `exit_guardian_relationship -> nurture.family_care.exit_guardian_relationship`. It uses `strong_authorization`, `scenario_direct_empty_v1`, exact current Guardian RoleAssignment/version, last-current-Guardian denial, and one transaction covering role revoke, actor-issued pending Co-Guardian intent cancellation, actor-owned Grant/dependent fence, Execution, and explicit `[]`. Other-owner Grants remain unchanged and no attention seed is created.
