...
| --- | --- | --- |
| Delivery CI bootstrap | PASS AFTER REPAIR / PR RE-RUN PENDING | PR #3 run `29419715925` reached all four execution jobs but failed before Nurture install because `pnpm --dir My-Workflow-Base/templates/web-workbench install` reported `Scope: all 5 workspace projects` and left the excluded standalone template without `node_modules`. Adding `--ignore-workspace` to `prepare:pinned-source` and all four CI jobs makes the dependency boundary explicit. A fresh `/tmp` checkout installed the template from its own frozen lockfile and built successfully; local `pnpm prepare:pinned-source` also passes with all exact pins unchanged. |
