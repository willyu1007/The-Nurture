...
| --- | --- | --- |
| Delivery CI bootstrap | PASS AFTER REPAIR / PR RE-RUN PENDING | PR #3 run `29419715925` failed before Nurture install because the excluded standalone web-workbench resolved the parent workspace; run `29420192609` proved that fix in all four execution jobs and passed both DB gates, then exposed missing My-Chat workspace/Prisma preparation only in the two jobs that typecheck X5 cross-repo sources. The final workflow uses `--ignore-workspace` for the template and prepares exact-revision My-Chat in those two jobs. A fresh three-repository `/tmp` checkout completes both installs, both Prisma generations, and Nurture typecheck; local lint and `175/175` unit tests pass with exact pins unchanged. |
