...
| IB-D1 | Participant identity and surface role resolution | **LOCKED, REFINED BY PILOT-0-B3-0/B3-1a:** One `NurtureParticipant` per `(workspaceId, myChatUserId)` remains canonical. Guardian uses generic AI Chat/family board/family workbench with accepted Guardian action availability; caregiver uses Nurture Chat/teacher board; institution admin uses institution board/workbench and not Nurture Chat; technical operator stays outside Nurture business surfaces. Nurture resolves role/scope/target/business direction/output on every surface. | Caregiver/institution/operator action matrices and exact manifest/adapter identifiers remain open. |
