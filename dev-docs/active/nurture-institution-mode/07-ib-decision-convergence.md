...
**Convergence status:** LOCKED on 2026-07-08; B3-1 refinements plus complete B3-2a-d cross-surface continuity semantics added on 2026-07-16.
...
- Unsubmitted text/form/AI drafts are actor- and surface-local and never become cross-surface/device/account state. Leaving a non-empty draft requires explicit stay/discard; Host transcript and My-Chat PublicDraft/Workflow artifact drafts cannot serve as Nurture action drafts. Committed continuity comes from Nurture business facts and refs-only Execution replay, while role surfaces owner-query current history. Transitions may add only generic `current|recent|history` view mode to route class, never business rows, ids, filters, cursors, bodies, or action state.
