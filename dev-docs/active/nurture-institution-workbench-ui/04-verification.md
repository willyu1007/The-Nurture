...
刻意未采纳参考实现的 `signOutHref`：尚无认证，指向不存在的登出页会是又一个死入口。
（`sections` 起初也判为无用，后因 hub 改为 home 项而必需——见下节。）
...
## 去掉重复的概览入口（2026-08-17，用户指出）

`home`（The Nurture）与 nav 项「概览台」都指向 `/nurture/overview`——**两个控件做同一件事**。
这也是更早那次「双高亮」的真正根因：上一轮把 home 挪到 `/` 只是治标。

改法：删掉「概览台」nav 项，home 变成 `概览` 并指回 `/nurture/overview`。
与姊妹实现一致——The-Education 的 `home` 就是它的看板。

同时按用户意见把 `入园流程` 并入「园区管理」：产品契约把当前 InstitutionWorkflow
的范围就定义为园区管理，所以它属于该类别；把它单拎出来只是因为"用得最多"，
那是使用频率而非类别，参考实现也把高频的「作业」放在分组内。

**连带发现**：`activeCrumb` 只遍历 `groups` 与 `sections`，**不看 home**。
删掉 nav 项后概览页会失去当前面包屑（而加粗面包屑就是页面标题）。
补一条 `sections` 解决——这正是 sections 的用途，也推翻了上一节"sections 对本工作台无用"的判断。

复测三个路由：

| 路由 | 高亮项 | 面包屑 |
| --- | --- | --- |
| `/nurture/overview` | 概览 | The Nurture › 概览 |
| `/nurture/queue` | 入园流程5 | The Nurture › 入园流程 |
| `/nurture/queue/[ref]` | 入园流程5 | The Nurture › 入园流程 › 豆豆 · 托大班 · … |

三处均只有一个高亮项，详情页的三级面包屑完整。全新 tab 零控制台错误。
