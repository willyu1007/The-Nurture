...
- 盘点 T-002 现有 family-care message/item/receipt 能力。
...
- 不持久化 prepared draft；execute 重交 typed input 并校验 canonical hash，ref 不包含 raw body/PII。
...
- prepare 将精确 target、capability-specific concurrency precondition、actor/scope、
  canonical input hash 与 expiry 绑定进 `confirmationRef`。
- acknowledge 使用精确 work-state version；reply 使用 replyable lifecycle/authority
  heads，兼容的新增回复不构成 stale。precondition 与 stable command identity
  仍分别负责状态安全和 transport retry/exact replay。
...
- 将 `executionDisposition` 放在调用外壳，将稳定 `resultRef`、business outcome、
  capability-specific output、receipt refs 与 invalidation scopes 放在不可变
  `committedResult`；replay 不重新计算历史业务结果。
...
- reply 输出稳定 CareReply/Message/Receipt refs、sequence、`first_response |
  additional_response` 与 Attention `resolved | unchanged`；不同 command 不互相去重。
...
- reply 不要求由 acknowledge actor 完成；任一当前属于同一精确 CareGroup 且通过
  原始 Grant/Enrollment/policy reread 的合格照护者都可以追加一条或多条回复。
...
- 由 Nurture 固定推导 family-care data class、question category、today-attention urgency、family-to-org direction、ack/reply 要求、空附件、author、current Grant、route、safe summary 与 command identity。
...
- 重试不产生重复消息或重复事项。
- 上下文续接只允许引用同一 ChildCareProcess、同一 Enrollment、当前可读且已 replied 的源 Item；关系不继承 Grant、authority、owner、SLA、状态或幂等身份。
- 新续接 Item 使用当前 Grant 和新的 business command identity；源 Item 后续不可读时隐藏关系，但不改变新 Item 的可用性。
...
- `redact_family_care_message` 只允许确切作者通过一次显式、effect-labeled 手势
  不可逆移除正文/附件及其 correction versions；Message tombstone、Receipt、
  Event、Execution 与 server-owned redaction reason 保留。
- policy/safety/admin redaction 使用独立系统 actor/capability，不伪装成作者命令。
...
- 验证两名同班照护者并发 reply 时两条独立 command 均提交，并按数据库时间/稳定
  sequence 排序；同一 command retry 只 exact replay 一条回复。
...
- 验证 prepare 四分支、五分钟 TTL、单次新 effect、input hash mismatch、target/authority/version drift 与跨主体/跨 surface 拒绝。
...
- 验证 replay 仅改变调用外壳判定，`committedResult` 的 result/output/receipt refs
...
- 验证同一 active/appendable Item 可接收多个独立 reply、新 Item 使用当前 Grant/
  新 command identity，以及上下文续接关系不产生 authority/state/SLA/dependency
  继承。
