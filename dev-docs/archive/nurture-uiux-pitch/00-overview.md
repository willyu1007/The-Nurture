# Overview — Nurture 六界面 UIUX 初稿 + Presenter Contract Handoff
...
2. **Presenter / Action contract handoff**：把六界面设计意图映射到 exact surface contract，
   交由 My-Chat T-036 继续组件与交互开发。早期园长共创 deck 已在项目上线前主动移除，
   不再作为交付物或实现输入。
...
## 六个 surface（历史 demo 主线，与 Pilot-0-B 锁定表面对应）
...
- 不做任何代码/schema/manifest/runtime 实现（T-002 门禁不变，本任务只交付设计与 contract handoff）。
...
- [x] 六个产品 surface 的 UIUX 初稿已完成；旧 9 页 HTML 在上线前清理，不再作为交付物。
...
契约化终态）。旧 deck 的版式与编辑能力只保留在决策历史中；当前实现只消费该 UX contract
与 presenter/action handoff。其余要点速览（详见契约）：
- 品牌：全线 morethan tokens（与 My-Chat / web-workbench 同源）；松林绿降级为场景语义色。
  （早期"松林奶油"主配色已于第五轮被 morethan 替代。）
- 历史 demo 形态：家长/老师两台"整机可操作"手机 + 机构双端静态页；该形态仅用于追溯，
  不再有可执行 deck，也不是当前实现规格。
