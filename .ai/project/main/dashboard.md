# Project Dashboard

Project: `main`

## Focus (index)
- Keep this section concise.
- Do not place semantic extraction body here; semantic details live in `feature-map.md` `Semantic Feature Briefs`.

### Current Focus
- Primary feature: F-002 Institution ecology
- Goal this cycle: repair Wave 4 P2 transaction-atomic owner authorization,
  adopt the repaired My-Chat private-anchor/idempotency pin, and preserve
  decision-complete Pilot-0-C plus design-locked Pilot-0-D while every
  runtime/environment activation remains unauthorized.
- Brief reference: `feature-map.md` → `F-002 Institution ecology`

### Coordination Signals
- Blocking dependency: six `TR-P0` plus three `TR-P1` traffic-readiness blockers, strict C30-C35 then C40-C45 qualification, D implementation/complete-candidate assembly, and Pilot-0-E are required before any Pilot-1 decision. The locked ECS/Compose path makes private Alibaba ACR a Pilot-1 prerequisite, not a current action.
- Wave 4 quality gate: CI-green Nurture `bfadbaf` is not qualified until
  authority reread and receipt persistence share one transaction; My-Chat must
  also remove stable anchors from client DTOs and repair Host
  idempotency/version concurrency.
- Decision deadline: before any DB apply, artifact publication, secret configuration, capability/manifest change, external pilot traffic, staging, production, or GA action.

### Next Governance Checkpoint
- Date: after Pilot-0-D design lock and before any implementation or Pilot-0-E/Pilot-1 authorization.
- Expected output: reviewed replacement P1/P2 pins with disposable-PostgreSQL
  authority-revoke concurrency evidence, followed by the locked C-3/C-4/D
  readiness review; no DB, artifact, secret, environment, activation, or
  traffic effect unless separately approved.

## Notes (manual)
- Keep human notes here. Everything below the AUTO section is generated by sync.

<!-- AUTO-GENERATED:START dashboard -->
## Summary

- Tasks: 3 (planned: 0, in-progress: 2, blocked: 0, done: 1, archived: 0)

## Recent tasks

| Task | Status | Feature | Dev Docs |
| --- | --- | --- | --- |
| T-002 nurture-institution-mode | in-progress | F-002 | dev-docs/active/nurture-institution-mode |
| T-003 nurture-uiux-pitch | in-progress | F-002 | dev-docs/active/nurture-uiux-pitch |
| T-001 nurture-mvp | done | F-001 | dev-docs/active/nurture-mvp |
<!-- AUTO-GENERATED:END dashboard -->
