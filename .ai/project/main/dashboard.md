...
- Implementation mainline: T-005 G2 Exit is PASS and the task is done. Exact
  ...
  T-006 G3-0 is frozen as `G3_0_FREEZE_PASS`; G3-A, G3-B1 and G3-C1 are
  delivered and the current critical lane is G3-D publish/release. Every
  checkpoint rotated the artifact additively, now at
  `nurture.surface-contract@1.11.0` / `sha256:7da48739…` — shared core and every
  T-005 capability slice stay byte-identical, so the G2 Exit evidence is preserved.
  The G3-C2 face matcher remains unimplemented and unregistered.
...
- Date: T-006 G3-D publish and release loop.
- Delivered: G3-0 freeze PASS plus G3-A, G3-B1 and G3-C1 — the role-safe boards,
  capture-to-draft, and the versioned ContentSafetyPolicy with manual media
  attribution, group-photo exposure gating and derived publish eligibility, on
  the additively rotated `1.11.0` artifact.
- Expected output: per-target `PublicationRelease`, revision freeze on first
  commit, partial-result and retry semantics. Real policy-backed scheduling
  stays blocked on the T-007 provider. No Candidate, secret, persistent
  environment, activation or traffic effect.
