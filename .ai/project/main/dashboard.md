...
- Implementation mainline: T-005 G2-A, G2-B and G2-C are PASS. The current exact
  provider root is `nurture.surface-contract@1.8.0` / `sha256:4fe91e13…` with
  eleven capabilities. Execute the T-005 G2 Exit Gate against the pinned owner path;
  preserve the single-writer/default-off fences and do not infer T-006 adoption.
...
- Stage G2: T-005 Exit requires G2-A Core CareInteraction Loop, G2-B lifecycle/Admin
  ...
  A/B/C are now checkpoint PASS; final cross-owner/cutover Exit evidence remains.
  T-006 may continue unrelated board work, but its `direct_interaction_required` route
  stays unavailable until it adopts the exact provider in G3-E.
...
- Execution backbone (2026-08-01): the recommended single-mainline order is
  ...
  T-008 Beta Profile v0) → T-005 G2-B → G2-C (complete) → G2 Exit →
...
- Date: T-005 G2 Exit Qualification.
- Delivered at W0: T-004 closure, G4-0A inventory PASS, G4-0B publication-policy
  contract freeze, and Beta Profile v0 freeze.
- Expected output: one complete G2 handoff binding A/B/C to the real pinned owner path,
  exact `1.8.0` artifact, single-writer/cutover census and remaining cross-owner
  negatives. No Candidate, secret, persistent environment, activation or traffic effect.
