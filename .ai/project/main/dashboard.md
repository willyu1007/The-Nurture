...
- Implementation mainline: T-005 G2 Exit is PASS and the task is done. Exact
  ...
  `nurture.surface-contract@1.12.0` / `sha256:a9dcd5c8…` — shared core and every
...
- Date: T-006 G3-E integration qualification.
- Delivered: G3-0 freeze PASS plus G3-A through G3-D — role-safe boards,
  capture-to-draft, manual content/media safety, and the publish/release loop
  with per-target releases, revision freeze, partial results and post-release
  safety actions, on the additively rotated `1.12.0` artifact. The G3 adoption
  set is closed at 35 capabilities.
- Expected output: formal NestJS ingress plus the real pinned owner path on
  disposable PostgreSQL, the negative matrix, T-005 G2-C joint qualification
  and the T-007 provider-backed schedule/release. No Candidate, secret,
  persistent environment, activation or traffic effect.
