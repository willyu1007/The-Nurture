...
- Related Tasks: T-004 (`done`); T-005 (`archived`); T-006 (`in-progress`);
  T-007～T-008 (`planned`).
- Next Checkpoint: close T-006/G3. Self-prep is complete at
  `nurture.surface-contract@1.15.0`; formal ingress admits 25 actions and 9
  queries, with only `reschedule_publish_process` waiting on the exact T-007
  publication-policy provider. Complete the T-007/T-006 policy joint run and
  the T-005/T-006 G2-C joint run, then issue the exact T-006 Beta Profile
  Handoff. T-008, deployment, activation and external traffic remain closed.
